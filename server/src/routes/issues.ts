...
export function issueRoutes(db: Db, storage: StorageService) {
  ...
  async function assertCanManageIssueApprovalLinks(req: Request, res: Response, companyId: string) {
    ...
    if (!req.actor.agentId) {
      res.status(403).json({ error: t("agents.authRequired") });
      return false;
    }
    const actorAgent = await agentsSvc.getById(req.actor.agentId);
    if (!actorAgent || actorAgent.companyId !== companyId) {
      res.status(403).json({ error: t("generic.forbidden") });
      return false;
    }
    if (actorAgent.role === "ceo" || Boolean(actorAgent.permissions?.canCreateAgents)) return true;
    res.status(403).json({ error: t("issues.missingPermissionLinkApprovals") });
    return false;
  }
  ...
  async function assertCanAssignTasks(req: Request, companyId: string) {
    ...
    if (req.actor.type === "board") {
      ...
      if (!allowed) throw forbidden(t("issues.missingPermissionTasksAssign"));
      return;
    }
    if (req.actor.type === "agent") {
      ...
      throw forbidden(t("issues.missingPermissionTasksAssign"));
    }
    throw unauthorized();
  }
  ...
  function requireAgentRunId(req: Request, res: Response) {
    ...
    res.status(401).json({ error: t("issues.agentRunIdRequired") });
    return null;
  }
  ...
  router.get("/companies/:companyId/issues", async (req, res) => {
    ...
    if (assigneeUserFilterRaw === "me" && (!assigneeUserId || req.actor.type !== "board")) {
      res.status(403).json({ error: t("issues.assigneeUserMeRequiresBoard") });
      return;
    }
    if (touchedByUserFilterRaw === "me" && (!touchedByUserId || req.actor.type !== "board")) {
      res.status(403).json({ error: t("issues.touchedByUserMeRequiresBoard") });
      return;
    }
    if (unreadForUserFilterRaw === "me" && (!unreadForUserId || req.actor.type !== "board")) {
      res.status(403).json({ error: t("issues.unreadForUserMeRequiresBoard") });
      return;
    }
  ...
  router.delete("/labels/:labelId", async (req, res) => {
    ...
    if (!existing) {
      res.status(404).json({ error: t("issues.labelNotFound") });
      return;
    }
  ...
  router.get("/issues/:id/documents/:key", async (req, res) => {
    ...
    if (!keyParsed.success) {
      res.status(400).json({ error: t("issues.documentKeyInvalid"), details: keyParsed.error.issues });
      return;
    }
    const doc = await documentsSvc.getIssueDocumentByKey(issue.id, keyParsed.data);
    if (!doc) {
      res.status(404).json({ error: t("issues.documentNotFound") });
      return;
    }
    res.json(doc);
  });
  ...
  router.delete("/issues/:id/documents/:key", async (req, res) => {
    ...
    assertCompanyAccess(req, issue.companyId);
    if (req.actor.type !== "board") {
      res.status(403).json({ error: t("issues.boardAuthRequired") });
      return;
    }
    const keyParsed = issueDocumentKeySchema.safeParse(String(req.params.key ?? "").trim().toLowerCase());
    if (!keyParsed.success) {
  ...
  router.post("/issues/:id/work-products", validate(createIssueWorkProductSchema), async (req, res) => {
    ...
    if (!product) {
      res.status(422).json({ error: t("issues.workProductInvalid") });
      return;
    }
    const actor = getActorInfo(req);
    await logActivity(db, {
  ...
  router.patch("/work-products/:id", validate(updateIssueWorkProductSchema), async (req, res) => {
    ...
    if (!existing) {
      res.status(404).json({ error: t("issues.workProductNotFound") });
      return;
    }
    assertCompanyAccess(req, existing.companyId);
  ...
  router.post("/issues/:id/read", async (req, res) => {
    ...
    if (!req.actor.userId) {
      res.status(403).json({ error: t("issues.boardUserContextRequired") });
      return;
    }
  ...
  router.post("/issues/:id/checkout", validate(checkoutIssueSchema), async (req, res) => {
        ...
        res.status(409).json({
          error:
            project.pauseReason === "budget"
              ? t("issues.projectPausedBudget")
              : t("issues.projectPaused"),
        });
        return;
      }
    }
    ...
    if (req.actor.type === "agent" && req.actor.agentId !== req.body.agentId) {
      res.status(403).json({ error: t("issues.agentCheckoutSelfOnly") });
      return;
    }
  ...
  router.get("/issues/:id/comments/:commentId", async (req, res) => {
    ...
    if (!comment || comment.issueId !== id) {
      res.status(404).json({ error: t("issues.commentNotFound") });
      return;
    }
    res.json(comment);
  });
  ...
  router.post("/issues/:id/comments", validate(addIssueCommentSchema), async (req, res) => {
    ...
    if (interruptRequested) {
      if (req.actor.type !== "board") {
        res.status(403).json({ error: t("issues.boardInterruptOnly") });
        return;
      }
  ...
  router.post("/companies/:companyId/issues/:issueId/attachments", async (req, res) => {
    ...
    if (issue.companyId !== companyId) {
      res.status(422).json({ error: t("issues.notBelongCompany") });
      return;
    }
    ...
    try {
      ...
      if (err instanceof multer.MulterError) {
        if (err.code === "LIMIT_FILE_SIZE") {
          res.status(422).json({ error: t("issues.attachmentExceedsLimit", { limit: MAX_ATTACHMENT_BYTES }) });
          return;
        }
    ...
    if (!file) {
      res.status(400).json({ error: t("issues.attachmentMissingFile") });
      return;
    }
    const contentType = (file.mimetype || "").toLowerCase();
    if (!isAllowedContentType(contentType)) {
      res.status(422).json({ error: t("issues.attachmentUnsupportedType", { contentType: contentType || "unknown" }) });
      return;
    }
    if (file.buffer.length <= 0) {
      res.status(422).json({ error: t("issues.attachmentEmpty") });
      return;
    }

    const parsedMeta = createIssueAttachmentMetadataSchema.safeParse(req.body ?? {});
    if (!parsedMeta.success) {
      res.status(400).json({ error: t("issues.attachmentMetadataInvalid"), details: parsedMeta.error.issues });
      return;
    }
  ...
  router.get("/attachments/:attachmentId/content", async (req, res, next) => {
    ...
    if (!attachment) {
      res.status(404).json({ error: t("issues.attachmentNotFound") });
      return;
    }
