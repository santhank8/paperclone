...
describe("sanitizeBranchName", () => {
  ...
  it("preserves 120-char limit on Unix", () => {
    const long = "DSPA-1558-" + "a".repeat(200);
  ...
  it("caps branch name at 40 chars on Windows", () => {
    const long = "DSPA-1558-worktree-creation-fails-on-windows-branch-name-deep-repo-paths-exceed-max-path-260-chars";
  ...
  it("truncates at word boundary (hyphen) on Windows", () => {
    const input = "DSPA-1558-worktree-creation-fails-on-windows-overflow";
  ...
  it("sanitizes special characters before truncating", () => {
    const input = "DSPA-100/fix: the (big) problem with spaces & symbols!!!";
    const result = sanitizeBranchName(input, { platform: "win32" });
