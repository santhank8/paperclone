...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("reaps a detached local run after the stale threshold even if the child pid is still alive", async () => {
    const child = spawnAliveProcess();
    childProcesses.add(child);
    expect(child.pid).toBeTypeOf("number");
    ...
    await db
      ...
      .set({
        ...
        error: `Lost in-memory process handle, but child pid ${child.pid} is still alive`,
