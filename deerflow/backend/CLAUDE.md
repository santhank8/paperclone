...
**Lead Agent** (`deerflow/agents/lead_agent/agent.py`):
...
Middlewares execute in strict order in `deerflow/agents/lead_agent/agent.py`:
...
### Sandbox System (`deerflow/sandbox/`)

**Interface**: Abstract `Sandbox` with `execute_command`, `read_file`, `write_file`, `list_dir`
**Provider Pattern**: `SandboxProvider` with `acquire`, `get`, `release` lifecycle
**Implementations**:
- `LocalSandboxProvider` - Singleton local filesystem execution with path mappings
- `AioSandboxProvider` (`deerflow/community/`) - Docker-based isolation
...
**Sandbox Tools** (in `deerflow/sandbox/tools.py`):
...
**Community tools** (`deerflow/community/`):
- `tavily/` - Web search (5 results default) and web fetch (4KB limit)
- `jina_ai/` - Web fetch via Jina reader API with readability extraction
- `firecrawl/` - Web scraping via Firecrawl API
- `image_search/` - Image search via DuckDuckGo

### MCP System (`deerflow/mcp/`)
...
### IM Channels System (`deerflow/channels/`)
...
### Reflection System (`deerflow/reflection/`)
...
### Embedded Client (`deerflow/client.py`)
