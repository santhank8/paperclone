...
class TestMemoryFilePath:
    def test_global_memory_path(self, tmp_path):
        ...
        path = storage._get_file_path(None)
    ...
    def test_agent_memory_path(self, tmp_path):
        ...
        storage = FileMemoryStorage(base_dir=tmp_path)
        path = storage._get_file_path("code-reviewer")
    ...
    def test_different_paths_for_different_agents(self, tmp_path):
        ...
        storage = FileMemoryStorage(base_dir=tmp_path)
        path_global = storage._get_file_path(None)
        path_a = storage._get_file_path("agent-a")
        path_b = storage._get_file_path("agent-b")
...
def _make_test_app(tmp_path: Path):
    ...
    from deerflow.gateway.routers.agents import router
...
def agent_client(tmp_path):
    ...
    with patch("deerflow.config.agents_config.get_paths", return_value=paths_instance), patch("deerflow.gateway.routers.agents.get_paths", return_value=paths_instance):
