...
from deerflow.gateway.routers.mcp import McpConfigResponse
from deerflow.gateway.routers.memory import MemoryConfigResponse, MemoryStatusResponse
from deerflow.gateway.routers.models import ModelResponse, ModelsListResponse
from deerflow.gateway.routers.skills import SkillInstallResponse, SkillResponse, SkillsListResponse
from deerflow.gateway.routers.uploads import UploadResponse
    ...
    def test_install_skill(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("deerflow.gateway.routers.skills._validate_skill_frontmatter", return_value=(True, "OK", "my-skill")),
            ):
    ...
    def test_upload_files_reuses_single_executor_inside_event_loop(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("deerflow.gateway.routers.uploads.CONVERTIBLE_EXTENSIONS", {".pdf"}),
                patch("deerflow.gateway.routers.uploads.convert_file_to_markdown", side_effect=fake_convert),
...
class TestScenarioSkillInstallAndUse:
    ...
    def test_install_then_toggle(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("deerflow.skills.loader.get_skills_root_path", return_value=skills_root),
                patch("deerflow.gateway.routers.skills._validate_skill_frontmatter", return_value=(True, "OK", "my-analyzer")),
            ):
    ...
    def test_upload_convertible_file_conversion_failure(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch.object(DeerFlowClient, "_get_uploads_dir", return_value=uploads_dir),
                patch("deerflow.gateway.routers.uploads.CONVERTIBLE_EXTENSIONS", {".pdf"}),
                patch("deerflow.gateway.routers.uploads.convert_file_to_markdown", side_effect=Exception("conversion failed")),
            ):
