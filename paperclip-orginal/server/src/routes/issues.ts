...
export function issueRoutes(db: Db, storage: StorageService) {
  ...
  function hasNonEmptyLabelIds(value: unknown): value is string[] {
    return Array.isArray(value) && value.length > 0;
  }

  function hasIssueCompletionAuthority(metadata: unknown) {
    if (typeof metadata !== "object" || metadata == null || Array.isArray(metadata)) {
      return false;
    }
    return (metadata as Record<string, unknown>).issueCompletionAuthority === true;
  }

  async function applyIssueMutationPolicy(
    req: Request,
    input: {
      mode: "create" | "update";
      companyId: string;
      fields: Record<string, unknown>;
      existing?: { id: string; identifier: string; title: string };
    },
  ) {
    const nextFields = { ...input.fields };
    const nextLabelIds = nextFields.labelIds;

    if (input.mode === "create" && !hasNonEmptyLabelIds(nextLabelIds)) {
      throw unprocessable("Issues must include at least one label");
    }
    if (input.mode === "update" && nextLabelIds !== undefined && !hasNonEmptyLabelIds(nextLabelIds)) {
      throw unprocessable("Issue label updates must include at least one label");
    }

    if (input.mode !== "update" || nextFields.status !== "done" || req.actor.type !== "agent") {
      return { nextFields, activity: null as null | Record<string, unknown> };
    }

    const actorAgent = req.actor.agentId ? await agentsSvc.getById(req.actor.agentId) : null;
    if (actorAgent && actorAgent.companyId === input.companyId && hasIssueCompletionAuthority(actorAgent.metadata)) {
      return { nextFields, activity: null as null | Record<string, unknown> };
    }

    // Completion authority is granted via PATCH /api/agents/:id metadata.issueCompletionAuthority=true.
    nextFields.status = "in_review";
    return {
      nextFields,
      activity: {
        action: "issue.done_redirected",
        details: {
          identifier: input.existing?.identifier,
          issueTitle: input.existing?.title,
  ...
  router.post("/companies/:companyId/issues", validate(createIssueSchema), async (req, res) => {
    ...
    const actor = getActorInfo(req);
    const { nextFields } = await applyIssueMutationPolicy(req, {
      mode: "create",
      companyId,
      fields: req.body as Record<string, unknown>,
    });
    const createFields = nextFields as Parameters<typeof svc.create>[1];
  ...
  router.patch("/issues/:id", validate(updateIssueSchema), async (req, res) => {
    ...
    const policyResult = await applyIssueMutationPolicy(req, {
      mode: "update",
      companyId: existing.companyId,
      fields: updateFields as Record<string, unknown>,
      existing: {
        id: existing.id,
    ...
    const normalizedUpdateFields = policyResult.nextFields as Parameters<typeof svc.update>[1];
    let issue;
    try {
      issue = await svc.update(id, normalizedUpdateFields);
    } catch (err) {
    ...
    if (policyResult.activity) {
      await logActivity(db, {
        ...
        agentId: actor.agentId,
        runId: actor.runId,
        action: policyResult.activity.action as string,
        entityType: "issue",
        entityId: issue.id,
        details: policyResult.activity.details as Record<string, unknown>,
      });
    }

    // Build activity details with previous values for changed fields
    const previous: Record<string, unknown> = {};
    for (const key of Object.keys(normalizedUpdateFields)) {
      if (key in existing && (existing as Record<string, unknown>)[key] !== (normalizedUpdateFields as Record<string, unknown>)[key]) {
