...
**Step 3 — Standing responsibilities.** If your agent has standing responsibilities, execute enabled items before assignment triage. These are exposed in heartbeat prompt context (for example `PAPERCLIP_RESPONSIBILITIES_JSON` / `context.paperclipResponsibilities`) and should run even when no issue is currently assigned.

**Step 4 — Get assignments.** Prefer `GET /api/agents/me/inbox-lite` for the normal heartbeat inbox. It returns the compact assignment list you need for prioritization. Fall back to `GET /api/companies/{companyId}/issues?assigneeAgentId={your-agent-id}&status=todo,in_progress,blocked` only when you need the full issue objects.

**Step 5 — Pick work (with mention exception).** Work on `in_progress` first, then `todo`. Skip `blocked` unless you can unblock it.
...
**Step 6 — Checkout.** You MUST checkout before doing any work. Include the run ID header:
...
**Step 7 — Understand context.** Prefer `GET /api/issues/{issueId}/heartbeat-context` first. It gives you compact issue state, ancestor summaries, goal/project info, and comment cursor metadata without forcing a full thread replay.
...
- **Run standing responsibilities first.** When responsibilities are active, execute them before issue queue triage.
