...
function workspaceLabel(workspace: Workspace): string {
  ...
  if (!baseLabel) {
    return workspace.isPrimary ? "(sem caminho de workspace) (primário)" : "(sem caminho de workspace)";
  }

  return workspace.isPrimary ? `${baseLabel} (primário)` : baseLabel;
}
...
export function FilesTab({ context }: PluginDetailTabProps) {
  ...
  async function handleCreate(kind: "file" | "directory") {
    ...
    const promptLabel = kind === "file" ? "Novo caminho de arquivo" : "Novo caminho de pasta";
      ...
      >
        ...
        >
          <div className="border-b border-border px-3 py-3">
            ...
            <div className="mt-3 grid gap-2">
              <input
                ...
                placeholder="Filtrar arquivos por nome ou caminho"
              />
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {isCreating ? "Criando..." : "Novo arquivo"}
                </button>
                <button
                  type="button"
                  className="rounded-md border border-input bg-background px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-accent disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <div className="min-h-0 flex-1 overflow-auto p-2">
            {selectedWorkspace ? (
              fileListLoading ? (
                <p className="px-2 py-3 text-sm text-muted-foreground">Carregando arquivos...</p>
                ...
                <p className="px-2 py-3 text-sm text-muted-foreground">Nenhum arquivo corresponde ao filtro atual.</p>
              ) : (
                <p className="px-2 py-3 text-sm text-muted-foreground">Nenhum arquivo encontrado neste workspace.</p>
              )
            ) : (
              <p className="px-2 py-3 text-sm text-muted-foreground">Selecione um workspace para navegar pelos arquivos.</p>
            )}
          </div>
        </div>
        <div
        ...
        >
          <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-border bg-card px-4 py-2">
            <div className="min-w-0">
              ...
              <div className="truncate text-sm text-foreground">{selectedPath ?? "Nenhum arquivo selecionado"}</div>
      ...
      {data.links.map((link) => {
        ...
        return (
          <a
            ...
            title={`Abrir ${link} no explorador de workspace`}
          >
            {link}
          </a>
        );
