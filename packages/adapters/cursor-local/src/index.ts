...
Use when:
- You want PrivateClip to run Cursor Agent CLI locally as the agent runtime
...
- PrivateClip auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$paperclip" and related skills on local runs.
- PrivateClip auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
