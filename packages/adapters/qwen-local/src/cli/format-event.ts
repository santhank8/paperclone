...
export function printQwenStreamEvent(raw: string, _debug: boolean): void {
  ...
  if (type === "system") {
    ...
    if (subtype === "error") {
      ...
      if (text) console.log(pc.red(`[qwen] error: ${text}`));
      return;
    }
    console.log(pc.blue(`[qwen] system: ${subtype || "event"}`));
    return;
  }
  ...
  if (type === "result") {
    ...
    if (subtype || isError) {
      console.log((isError ? pc.red : pc.blue)(`[qwen] result: subtype=${subtype} is_error=${isError ? "true" : "false"}`));
    }
    return;
  }
