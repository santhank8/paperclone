...
function parseAssistantMessage(messageRaw: unknown, ts: string): TranscriptEntry[] {
  ...
  for (const partRaw of content) {
    ...
    if (type === "tool_call") {
      ...
      const input =
        name === "run_shell_command" || name === "shellToolCall" || name === "shell"
...
function parseToolCallEvent(event: Record<string, unknown>, ts: string): TranscriptEntry[] {
  ...
  const isShellTool = toolName === "run_shell_command" || toolName === "shellToolCall" || toolName === "shell";
...
export function parseQwenStdoutLine(line: string, ts: string): TranscriptEntry[] {
  const normalized = normalizeQwenStreamLine(line);
