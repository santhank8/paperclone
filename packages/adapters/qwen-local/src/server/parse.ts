...
function isLikelyQwenFailureText(value: string): boolean {
  return /^\[api error:/i.test(value.trim()) ||
    /invalid\s+access\s+token|token\s+expired|not\s+authenticated|authentication\s+required|please\s+log\s+in|login\s+required|unauthorized|qwen\s+auth|请先登录|未登录|未认证/i.test(
      value,
    );
}
...
function collectEvents(stdout: string): Array<Record<string, unknown>> {
  ...
  for (const rawLine of stdout.split(/\r?\n/)) {
    const line = normalizeQwenStreamLine(rawLine).line;
    if (!line) continue;
...
export function parseQwenJsonl(stdout: string) {
  ...
  for (const event of collectEvents(stdout)) {
    ...
    if (type === "assistant") {
      const assistantLines = collectAssistantText(event.message);
      messages.push(...assistantLines);
      if (!errorMessage) {
        const assistantError = assistantLines.find((line) => isLikelyQwenFailureText(line));
        if (assistantError) errorMessage = assistantError.trim();
      }
      continue;
    }
    ...
    if (type === "result") {
      ...
      } else if (!errorMessage && isLikelyQwenFailureText(resultText)) {
...
export function detectQwenAuthRequired(input: {
  parsed: ReturnType<typeof parseQwenJsonl> | null;
  stdout: string;
  stderr: string;
}): { requiresAuth: boolean } {
  const haystack = [
    input.parsed?.errorMessage ?? "",
...
export function isQwenUnknownSessionError(stdout: string, stderr: string): boolean {
  ...
  return /unknown\s+session|session\s+.*\s+not\s+found|resume\s+.*\s+not\s+found|cannot\s+resume|failed\s+to\s+resume|no\s+saved\s+session\s+found\s+with\s+id/i.test(
    haystack,
  );
}
