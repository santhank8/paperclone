...
import { DEFAULT_QWEN_LOCAL_MODEL, DEFAULT_QWEN_BASE_URL } from "../index.js";
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const apiKey = resolveEnvValue(env, "DASHSCOPE_API_KEY") || process.env.DASHSCOPE_API_KEY || "";
  const model = asString(config.model, DEFAULT_QWEN_LOCAL_MODEL);
  const baseUrl = asString(config.baseUrl, DEFAULT_QWEN_BASE_URL);
  const maxTokens = asNumber(config.maxTokens);
  const temperature = asNumber(config.temperature);

  if (!apiKey) {
    await onLog("stderr", JSON.stringify({ type: "error", error: "DASHSCOPE_API_KEY 未设置" }));
    return {
      exitCode: 1,
      signal: null,
      timedOut: false,
      errorMessage: "DASHSCOPE_API_KEY 未设置",
      provider: "alibaba",
      biller: "dashscope",
      model,
      billingType: "api",
  ...
  return executeOpenAiCompatChat(
    { baseUrl, apiKey, model, maxTokens, temperature, provider: "alibaba", biller: "dashscope", adapterType: "qwen_local" },
    messages,
    { onLog },
  );
}
