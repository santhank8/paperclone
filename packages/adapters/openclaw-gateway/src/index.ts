...
Use when:
- You want PrivateClip to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the PrivateClip server.
...
- paperclipApiUrl (string, optional): absolute PrivateClip base URL advertised in wake text
...
- paperclip (object): standardized PrivateClip context added to every gateway agent request
- paperclip.workspace (object, optional): resolved execution workspace for this run
- paperclip.workspaces (array, optional): additional workspace hints PrivateClip exposed to the run
