...
export function printCodeBuddyStreamEvent(raw: string, _debug: boolean): void {
  const line = normalizeCodeBuddyStreamLine(raw).line;
  if (!line) return;
  ...
  if (type === "system") {
    ...
    if (subtype === "init") {
      ...
      const details = [sessionId ? `session: ${sessionId}` : "", model ? `model: ${model}` : ""]
        .filter(Boolean)
        .join(", ");
      console.log(pc.blue(`CodeBuddy init${details ? ` (${details})` : ""}`));
      return;
    }
