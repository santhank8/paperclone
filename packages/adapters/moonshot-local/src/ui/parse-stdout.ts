...
export function parseMoonshotStdoutLine(line: string, ts: string): TranscriptEntry[] {
  ...
  if (type === "system") {
    ...
    if (subtype === "init") {
      return [{ kind: "init", ts, model: asString(parsed.model, "moonshot"), sessionId: asString(parsed.session_id) }];
    }
    if (subtype === "error") {
