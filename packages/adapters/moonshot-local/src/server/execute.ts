...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const apiKey = resolveEnvValue(env, "MOONSHOT_API_KEY") || process.env.MOONSHOT_API_KEY || "";
  const model = asString(config.model, DEFAULT_MOONSHOT_LOCAL_MODEL);
  const baseUrl = asString(config.baseUrl, DEFAULT_MOONSHOT_BASE_URL);
  const maxTokens = asNumber(config.maxTokens);
  const temperature = asNumber(config.temperature);

  if (!apiKey) {
    await onLog("stderr", JSON.stringify({ type: "error", error: "MOONSHOT_API_KEY 未设置" }));
    return {
      exitCode: 1,
      signal: null,
      timedOut: false,
      errorMessage: "MOONSHOT_API_KEY 未设置",
      provider: "moonshot",
      biller: "moonshot",
      model,
      billingType: "api",
  ...
  return executeOpenAiCompatChat(
    { baseUrl, apiKey, model, maxTokens, temperature, provider: "moonshot", biller: "moonshot", adapterType: "moonshot_local" },
    messages,
    { onLog },
  );
}
