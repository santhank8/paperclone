...
export async function testEnvironment(ctx: AdapterEnvironmentTestContext): Promise<AdapterEnvironmentTestResult> {
  ...
  const model = (typeof ctx.config.model === "string" && ctx.config.model) || DEFAULT_MOONSHOT_LOCAL_MODEL;
  const baseUrl = (typeof ctx.config.baseUrl === "string" && ctx.config.baseUrl) || DEFAULT_MOONSHOT_BASE_URL;

  return testOpenAiCompatApiKey(baseUrl, apiKey, model, "moonshot_local", "MOONSHOT_API_KEY");
}
