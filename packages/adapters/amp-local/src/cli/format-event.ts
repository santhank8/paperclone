...
export function printAmpStreamEvent(raw: string, debug: boolean): void {
  const line = raw.trim();
  if (!line) return;
  ...
  if (type === "system" && parsed.subtype === "init") {
    ...
    console.log(pc.blue(`Amp initialized (model: ${model}${sessionId ? `, thread: ${sessionId}` : ""})`));
    return;
  }
  ...
  if (type === "result") {
    ...
    if (subtype.startsWith("error") || isError || errors.length > 0) {
      console.log(pc.red(`amp_result: subtype=${subtype || "unknown"} is_error=${isError ? "true" : "false"}`));
      if (errors.length > 0) {
        console.log(pc.red(`amp_errors: ${errors.join(" | ")}`));
      }
    }
    console.log(
      pc.blue(
