...
import { parseCopilotStreamJson, detectCopilotLoginRequired } from "./parse.js";
...
export async function testEnvironment(
  ...
  // Validate command is resolvable
  try {
  ...
  // Check GitHub auth indicators
  const githubToken = env.GITHUB_TOKEN ?? process.env.GITHUB_TOKEN;
  if (githubToken) {
    checks.push({
      code: "copilot_github_token_set",
      level: "info",
      message: "GITHUB_TOKEN is set; Copilot CLI can use it for authentication.",
    });
  } else {
    checks.push({
      code: "copilot_github_token_unset",
      level: "info",
      message: "GITHUB_TOKEN is not set; Copilot CLI will rely on `gh auth` or `copilot login`.",
    });
  }
  ...
  if (canRunProbe) {
    if (!commandLooksLike(command, "copilot")) {
      ...
      const parsedStream = parseCopilotStreamJson(probe.stdout);
      ...
      if (probe.timedOut) {
        checks.push({
          ...
          hint: "Retry the probe. If this persists, verify Copilot can run `-p 'Respond with hello.'` from this directory manually.",
        });
        ...
        checks.push({
          ...
          ...(detail ? { detail } : {}),
          hint: "Run `copilot login` or `gh auth login` in this environment, then retry the probe.",
        });
        ...
        checks.push({
          ...
          ...(hasHello
            ? {}
            : {
                hint: "Try the probe manually: `copilot -p 'Respond with hello.' --output-format json --allow-all-tools --no-ask-user`",
              }),
        });
      } else {
        checks.push({
          ...
          ...(detail ? { detail } : {}),
          hint: "Run `copilot -p 'Respond with hello.' --output-format json --allow-all-tools --no-ask-user` manually in this directory to debug.",
        });
      }
    }
  }
