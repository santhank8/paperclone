...
function resolveCursorBillingType(env: Record<string, string>): "api" | "subscription" {
  return hasNonEmptyEnvValue(env, "COPILOT_GITHUB_TOKEN") || hasNonEmptyEnvValue(env, "OPENAI_API_KEY")
    ? "api"
    : "subscription";
}
...
function resolveCursorBiller(
  ...
  if (billingType === "subscription") return "copilot";
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const buildArgs = (resumeSessionId: string | null, promptText: string) => {
    const args = ["-p", promptText, "--output-format", "json"];
    if (resumeSessionId) args.push("--resume", resumeSessionId);
    if (model) args.push("--model", model);
    if (autoTrustEnabled) args.push("--allow-all-tools");
