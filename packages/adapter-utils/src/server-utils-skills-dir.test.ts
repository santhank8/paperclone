...
describe("resolvePaperclipSkillsDir", () => {
  ...
  it("returns PAPERCLIP_SKILLS_DIR when set to an existing directory", async () => {
  ...
  it("env var takes priority over an otherwise-matching module-relative candidate", async () => {
    const envDir = path.join(tmpRoot, "env-skills");
    await fs.mkdir(envDir, { recursive: true });
    process.env.PAPERCLIP_SKILLS_DIR = envDir;

    // Also create a valid module-relative candidate — env var should still win.
    const repoRoot = path.join(tmpRoot, "repo");
    const moduleDir = path.join(repoRoot, "packages", "adapters", "x", "src", "server");
    const moduleRelativeSkills = path.join(repoRoot, "skills");
    await fs.mkdir(moduleDir, { recursive: true });
    await fs.mkdir(moduleRelativeSkills, { recursive: true });

    const resolved = await resolvePaperclipSkillsDir(moduleDir);
    expect(resolved).toBe(envDir);
    expect(resolved).not.toBe(moduleRelativeSkills);
  });

  it("falls back to ../../skills when env var is unset (published layout)", async () => {
  ...
  it("falls back to ../../../../../skills when env var is unset (workspace layout)", async () => {
    const repoRoot = path.join(tmpRoot, "repo");
  ...
  it("warns and falls back when env var points at a non-directory", async () => {
  ...
  it("warns and falls back when env var points at a non-existent path", async () => {
  ...
  it("treats whitespace-only env var as unset", async () => {
    ...
    const moduleDir = path.join(repoRoot, "packages", "adapters", "w", "src", "server");
    const skillsDir = path.join(repoRoot, "skills");
    await fs.mkdir(moduleDir, { recursive: true });
    await fs.mkdir(skillsDir, { recursive: true });
  ...
  it("honors additionalCandidates when module-relative candidates miss", async () => {
    const extraRoot = path.join(tmpRoot, "extra-skills");
    await fs.mkdir(extraRoot, { recursive: true });

    const moduleDir = path.join(tmpRoot, "orphan2", "src", "server");
    await fs.mkdir(moduleDir, { recursive: true });

    const resolved = await resolvePaperclipSkillsDir(moduleDir, [extraRoot]);
    expect(resolved).toBe(extraRoot);
  });
});
