...
function buildConnectionErrorMessage(input: {
  ...
  lines.push(
    "",
    "This usually means the PrivateClip server is not running, the configured URL is wrong, or the request is being blocked before it reaches PrivateClip.",
    "",
    "Try:",
    "- Start PrivateClip with `pnpm dev` or `pnpm paperclipai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If PrivateClip is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PAPERCLIP_API_URL\`.`,
  );
  return lines.join("\n");
}
