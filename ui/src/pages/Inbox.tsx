...
function runFailureMessage(t: any, run: Partial<HeartbeatRun>): string {
...
function getInboxIssueColumnLabels(t: any): Record<InboxIssueColumn, string> {
  return {
    status: t("page.inbox.columns.labels.status"),
    id: t("page.inbox.columns.labels.id"),
    assignee: t("page.inbox.columns.labels.assignee"),
    project: t("page.inbox.columns.labels.project"),
    workspace: t("page.inbox.columns.labels.workspace"),
    labels: t("page.inbox.columns.labels.labels"),
    updated: t("page.inbox.columns.labels.updated"),
  };
}

function getInboxIssueColumnDescriptions(t: any): Record<InboxIssueColumn, string> {
  return {
    status: t("page.inbox.columns.descriptions.status"),
    id: t("page.inbox.columns.descriptions.id"),
    assignee: t("page.inbox.columns.descriptions.assignee"),
    project: t("page.inbox.columns.descriptions.project"),
    workspace: t("page.inbox.columns.descriptions.workspace"),
    labels: t("page.inbox.columns.descriptions.labels"),
    updated: t("page.inbox.columns.descriptions.updated"),
  };
}
...
function issueActivityText(issue: Issue, t: any): string {
  return t("page.inbox.labels.updated", {
...
export function InboxIssueTrailingColumns({
  ...
  const userLabel = formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("page.inbox.labels.user");
    ...
    >
      {columns.map((column) => {
        if (column === "assignee") {
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">
              {t("page.inbox.labels.unassigned")}
            </span>
          );
        }
        ...
        if (column === "project") {
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">
              {t("page.inbox.labels.no_project")}
            </span>
          );
        }
      ...
      <div className="flex items-start gap-2 sm:items-center">
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>{t("page.inbox.labels.failed_run")}{linkedAgentName ? ` — ${linkedAgentName}` : ""}</>
              )}
            </span>
        ...
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
          ...
          >
            ...
            {isRetrying ? t("page.inbox.actions.retrying") : t("page.inbox.actions.retry")}
          </Button>
          {!showUnreadSlot && (
            <button
              type="button"
...
function JoinRequestInboxRow({
  ...
  const label =
    joinRequest.requestType === "human"
      ...
      : `${t("page.inbox.labels.agent_join_request")}${joinRequest.agentName ? `: ${joinRequest.agentName}` : ""}`;
  const showUnreadSlot = unreadState !== null;
  const showUnreadDot = unreadState === "visible" || unreadState === "fading";
      ...
      <div className="flex items-start gap-2 sm:items-center">
          ...
          <span className="hidden sm:inline-flex h-4 w-4 shrink-0 items-center justify-center self-center">
            {showUnreadDot ? (
              <button
                ...
                aria-label={t("page.inbox.actions.dismiss_from_inbox")}
              >
                <X className="h-3.5 w-3.5" />
              </button>
            ) : (
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{t("page.inbox.labels.requested_time_from_ip", { time: timeAgo(joinRequest.createdAt), ip: joinRequest.requestIp })}</span>
              {joinRequest.adapterType && <span>{t("page.inbox.labels.adapter", { type: joinRequest.adapterType })}</span>}
            </span>
          </span>
        </div>
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
...
export function Inbox() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("page.inbox.title") }]);
  }, [setBreadcrumbs, t]);
  ...
  const filteredWorkItems = useMemo(() => {
    ...
    return workItemsToRender.filter((item) => {
      ...
      if (item.kind === "failed_run") {
        ...
        const msg = runFailureMessage(t, run);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("page.inbox.errors.approve_failed"));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("page.inbox.errors.reject_failed"));
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("page.inbox.errors.approve_join_failed"));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("page.inbox.errors.reject_join_failed"));
    },
  });
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id) => {
      setActionError(err instanceof Error ? err.message : t("page.inbox.errors.archive_failed"));
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={InboxIcon} message={t("page.inbox.empty.selectCompany")} />;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <Input
              type="search"
              placeholder={t("page.inbox.search_placeholder")}
              value={searchQuery}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                ...
                {t("page.inbox.columns.show_hide")}
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-[300px] rounded-xl border-border/70 p-1.5 shadow-xl shadow-black/10">
              <DropdownMenuLabel className="px-2 pb-1 pt-1.5">
                <div className="space-y-1">
                  <div className="text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">
                    {t("page.inbox.columns.desktop_rows")}
                  </div>
                  <div className="text-sm font-medium text-foreground">
                    {t("page.inbox.columns.choose_visible")}
                  </div>
                </div>
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
                ...
                >
                  <span className="flex flex-col gap-0.5">
                    <span className="text-sm font-medium text-foreground">
                      {getInboxIssueColumnLabels(t)[column]}
                    </span>
                    <span className="text-xs leading-relaxed text-muted-foreground">
                      {getInboxIssueColumnDescriptions(t)[column]}
                    </span>
                  </span>
                </DropdownMenuCheckboxItem>
              ))}
              ...
              >
                {t("page.inbox.columns.reset")}
                <span className="ml-auto text-xs text-muted-foreground">{t("page.inbox.columns.default_list")}</span>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          {canMarkAllRead && (
            <>
              ...
              >
                {markAllReadMutation.isPending ? t("page.inbox.actions.marking") : t("page.inbox.actions.mark_all_read")}
              </Button>
              <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>{t("page.inbox.actions.mark_all_read")}?</DialogTitle>
                    <DialogDescription>
                      {t("page.inbox.dialogs.mark_all_read_description", { count: unreadIssueIds.length })}
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
                      <Button variant="outline" onClick={() => setShowMarkAllReadConfirm(false)}>
                        {t("page.inbox.dialogs.cancel")}
                      </Button>
                      <Button
                        onClick={() => {
                      ...
                      >
                        {t("page.inbox.actions.mark_all_read")}
                      </Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-[170px] text-xs">
              <SelectValue placeholder={t("page.inbox.categories.placeholder")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="everything">{t("page.inbox.categories.everything")}</SelectItem>
              <SelectItem value="issues_i_touched">{t("page.inbox.categories.issues_i_touched")}</SelectItem>
              <SelectItem value="join_requests">{t("page.inbox.categories.join_requests")}</SelectItem>
              <SelectItem value="approvals">{t("page.inbox.categories.approvals")}</SelectItem>
              <SelectItem value="failed_runs">{t("page.inbox.categories.failed_runs")}</SelectItem>
              <SelectItem value="alerts">{t("page.inbox.categories.alerts")}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {showApprovalsCategory && (
            ...
            >
                <SelectTrigger className="h-8 w-[170px] text-xs">
                  <SelectValue placeholder={t("page.inbox.approval_status.placeholder")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">{t("page.inbox.approval_status.all")}</SelectItem>
                  <SelectItem value="actionable">{t("page.inbox.approval_status.actionable")}</SelectItem>
                  <SelectItem value="resolved">{t("page.inbox.approval_status.resolved")}</SelectItem>
                </SelectContent>
            </Select>
          )}
        </div>
      ...
      {allLoaded && visibleSections.length === 0 && (
        <EmptyState
          ...
          message={
            searchQuery.trim()
              ? t("page.inbox.no_match_search")
              : tab === "mine"
              ? t("page.inbox.no_new_items")
              : tab === "unread"
              ? t("page.inbox.no_new_items")
              : tab === "recent"
                ? t("page.inbox.no_recent_items")
                : t("page.inbox.no_match_filters")
          }
        />
      )}
                ...
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      ...
                      {t("inbox.alerts.agentsError", { count: dashboard!.agents.error })}
                    </span>
                  </Link>
                  <button
                    type="button"
              ...
              {showBudgetAlert && (
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {t("inbox.alerts.budget", { percent: dashboard!.costs.monthUtilizationPercent })}
                    </span>
                  </Link>
                  <button
                    type="button"
