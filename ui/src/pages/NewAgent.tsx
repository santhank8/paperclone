...
export function NewAgent() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("newAgent.agents"), href: "/agents" },
      { label: t("newAgent.newAgent") },
    ]);
  }, [setBreadcrumbs, t]);

  useEffect(() => {
    if (isFirstAgent) {
      if (!name) setName(t("newAgent.defaultCeo"));
      if (!title) setTitle(t("newAgent.defaultCeo"));
    }
  }, [isFirstAgent, t]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      ...
      if (!selectedModel) {
        setFormError(t("newAgent.openCodeNeedsModel"));
        return;
      }
      if (adapterModelsError) {
        setFormError(
          adapterModelsError instanceof Error
            ...
            : t("newAgent.failedToLoadModels"),
        );
        return;
      }
      if (adapterModelsLoading || adapterModelsFetching) {
        setFormError(t("newAgent.openCodeLoading"));
        return;
      }
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="border border-border">
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">{t("newAgent.firstAgentNote")}</p>
          )}
          {formError && (
