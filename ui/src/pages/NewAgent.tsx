...
export function NewAgent() {
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : t("newAgent.failedCreate"));
    },
  });
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      ...
      if (!selectedModel) {
        setFormError(t("onboarding.openCodeRequiresModel"));
        return;
      }
      ...
      if (adapterModelsLoading || adapterModelsFetching) {
        setFormError(t("onboarding.openCodeModelsLoading"));
        return;
      }
      ...
      if (!discovered.some((entry) => entry.id === selectedModel)) {
        setFormError(
          discovered.length === 0
            ...
            : t("onboarding.openCodeModelUnavailable", { model: selectedModel }),
        );
        return;
      }
    }
              ...
              >
                {currentReportsTo ? (
                  <>
                    ...
                    {t("newAgent.reportsToAgent", { name: currentReportsTo.name })}
                  </>
                ) : (
                  <>
                    <User className="h-3 w-3 text-muted-foreground" />
                    {isFirstAgent ? t("newAgent.reportsToNa") : t("newAgent.reportsToPlaceholder")}
                  </>
                )}
              </button>
            </PopoverTrigger>
