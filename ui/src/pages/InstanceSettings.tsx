...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.heartbeatsPage.errors.update"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceSettings.heartbeatsPage.loading")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instanceSettings.heartbeatsPage.errors.load")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceSettings.heartbeatsPage.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">{t("instanceSettings.heartbeatsPage.description")}</p>
      </div>

      <div className="flex gap-4 text-sm text-muted-foreground">
        <span>
          <span className="font-semibold text-foreground">{activeCount}</span>{" "}
          {t("instanceSettings.heartbeatsPage.stats.active")}
        </span>
        <span>
          <span className="font-semibold text-foreground">{disabledCount}</span>{" "}
          {t("instanceSettings.heartbeatsPage.stats.disabled")}
        </span>
        <span>
          <span className="font-semibold text-foreground">{grouped.length}</span>{" "}
          {grouped.length === 1
            ? t("instanceSettings.heartbeatsPage.stats.company")
            : t("instanceSettings.heartbeatsPage.stats.companies")}
        </span>
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("emptyState.noHeartbeats")}
        />
      ) : (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive
                            ? t("instanceSettings.heartbeatsPage.badges.on")
                            : t("instanceSettings.heartbeatsPage.badges.off")}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("instanceSettings.heartbeatsPage.never")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("instanceSettings.heartbeatsPage.fullAgentConfig")}
                          >
                          ...
                          >
                            {saving
                              ...
                              : agent.heartbeatEnabled
                                ? t("instanceSettings.heartbeatsPage.actions.disable")
                                : t("instanceSettings.heartbeatsPage.actions.enable")}
                          </Button>
                        </span>
                      </div>
                    );
