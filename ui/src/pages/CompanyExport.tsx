  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("companyExport.failedLoadExportData"),
      });
    },
  });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        ...
        body: t("companyExport.exportedZipBody", {
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyExport.exportFailed"),
        body: err instanceof Error ? err.message : t("companyExport.failedBuildExportPackage"),
      });
    },
  });
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4 text-sm">
            <span className="font-medium">
              {selectedCompany?.name ?? t("companyExport.companyFallback")} {t("companyExport.exportLabel")}
            </span>
            <span className="text-muted-foreground">
              {t("companyExport.selectedCount", { selected: selectedCount, total: totalFiles, suffix: totalFiles === 1 ? "" : "s" })}
            </span>
            {warnings.length > 0 && (
              <span className="text-amber-500">
                {t("companyExport.warningCount", { count: warnings.length, suffix: warnings.length === 1 ? "" : "s" })}
              </span>
            )}
          </div>
          <Button
          ...
          >
            ...
            {downloadMutation.isPending
              ...
              : t("companyExport.exportCount", { count: selectedCount, suffix: selectedCount === 1 ? "" : "s" })}
          </Button>
        </div>
      </div>
