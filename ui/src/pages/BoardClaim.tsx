...
export function BoardClaimPage() {
  ...
  if (statusQuery.isLoading || sessionQuery.isLoading) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">加载认领挑战中...</div>;
  }
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">认领挑战不可用</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {statusQuery.error instanceof Error ? statusQuery.error.message : "挑战无效或已过期。"}
          </p>
        </div>
      </div>
    );
  ...
  if (!status) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">认领挑战不可用。</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          这将把您的用户提升为实例管理员，并将公司所有权访问从本地信任模式迁移。
        </p>
