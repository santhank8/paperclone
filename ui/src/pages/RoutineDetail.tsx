...
const concurrencyPolicyDescriptions: Record<string, string> = {
  coalesce_if_active: "当活跃运行仍在执行时，仅保留一个后续运行排队。",
  always_enqueue: "将每次触发都加入队列，即使多个运行堆积。",
  skip_if_active: "当例程已在活跃运行时，丢弃重叠的触发。",
};
const catchUpPolicyDescriptions: Record<string, string> = {
  skip_missed: "忽略例程或调度器暂停期间错过的调度窗口。",
  enqueue_missed_with_cap: "恢复后以限量批次补执行错过的调度窗口。",
};
const signingModeDescriptions: Record<string, string> = {
  bearer: "在 Authorization 头中使用共享的 Bearer 令牌。",
  hmac_sha256: "使用共享密钥对请求进行 HMAC SHA-256 签名。",
};
...
export function RoutineDetail() {
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: "保存例程失败",
        body: error instanceof Error ? error.message : "Paperclip 无法保存该例程。",
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: "更新例程失败",
        body: error instanceof Error ? error.message : "Paperclip 无法更新该例程。",
        tone: "error",
      });
    },
  });
  ...
  if (error || !routine) {
    return (
      <p className="pt-6 text-sm text-destructive">
        {error instanceof Error ? error.message : "未找到例程"}
      </p>
    );
  }
  ...
  const automationLabel = routine.status === "archived" ? "已归档" : automationEnabled ? "活跃" : "已暂停";
    ...
    <div className="max-w-2xl space-y-6">
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">请立即保存。Paperclip 不会再次显示此密钥值。</p>
          </div>
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
          <span className="text-sm font-medium">高级交付设置</span>
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending ? "正在添加..." : "添加触发器"}
              </Button>
            </div>
          </div>
