...
export function InviteLandingPage() {
  ...
  const adapterLabel = (type: string) => {
    switch (type) {
      case "claude_local":
        return `${t("onboardingAdapters.claudeCode")} (local)`;
      case "codex_local":
        return `${t("onboardingAdapters.codex")} (local)`;
      case "gemini_local":
        return `${t("onboardingAdapters.geminiCli")} (local)`;
      case "opencode_local":
        return `${t("onboardingAdapters.openCode")} (local)`;
      case "pi_local":
        return `${t("onboardingAdapters.pi")} (local)`;
      case "openclaw_gateway":
        return t("onboardingAdapters.openClawGateway");
      case "cursor":
        return `${t("onboardingAdapters.cursor")} (local)`;
      case "hermes_local":
        return t("onboardingAdapters.hermesAgent");
      case "process":
        return t("agentConfig.adapterLabels.process");
      case "http":
        return t("agentConfig.adapterLabels.http");
      default:
        return type;
    }
  };
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("inviteLanding.unavailableTitle")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.unavailableBody")}
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.bootstrapCompleteBody")}
          </p>
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.joinRequestPending")}
          </p>
          ...
          {claimSecret && claimApiKeyPath && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("inviteLanding.oneTimeClaimSecret")}</p>
          ...
          {(onboardingSkillUrl || onboardingSkillPath || onboardingInstallPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              ...
              {onboardingInstallPath && <p className="font-mono break-all">{t("inviteLanding.installTo")} {onboardingInstallPath}</p>}
            </div>
          )}
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo"
            ? t("inviteLanding.bootstrapTitle")
            : companyName
              ? t("inviteLanding.joinCompany", { company: companyName })
              : t("inviteLanding.joinPaperclipCompany")}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {invite.inviteType !== "bootstrap_ceo" && companyName
            ? `${t("inviteLanding.invitedToJoin", { company: companyName })} `
            : null}
          {t("inviteLanding.inviteExpires", { date: dateTime(invite.expiresAt) })}
        </p>
        ...
        {invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-5 flex gap-2">
            {availableJoinTypes.map((type) => (
              ...
              >
                {t("inviteLanding.joinAs", { type: t(`inviteLanding.${type}`) })}
              </button>
            ))}
          </div>
        )}
              ...
              >
                {joinAdapterOptions.map((type) => (
                  <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                    {adapterLabel(type)}{!ENABLED_INVITE_ADAPTERS.has(type) ? ` (${t("inviteLanding.comingSoon")})` : ""}
                  </option>
                ))}
              </select>
            </label>
        ...
        {requiresAuthForHuman && (
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            {t("inviteLanding.signInRequired")}
            <div className="mt-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/auth?next=${encodeURIComponent(`/invite/${token}`)}`}>{t("inviteLanding.signInCreate")}</Link>
              </Button>
            </div>
          </div>
        )}
