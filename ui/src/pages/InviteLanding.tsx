...
export function InviteLandingPage() {
  ...
  const acceptMutation = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : translateText("Failed to create invite"));
    },
  });

  if (!token) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">{t("invite.invalid")}</div>;
  }
  ...
  if (result?.kind === "bootstrap") {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{translateText("Bootstrap complete")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {translateText("The first instance admin is now configured. You can continue to the board.")}
          </p>
          <Button asChild className="mt-4">
            <Link to="/">{translateText("Open board")}</Link>
          </Button>
        </div>
      </div>
    );
  ...
  if (result?.kind === "join") {
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{translateText("Join request submitted")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {translateText("Your request is pending admin approval. You will not have access until approved.")}
          </p>
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
            {translateText("Request ID:")} <span className="font-mono">{payload.id}</span>
          </div>
          {claimSecret && claimApiKeyPath && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{translateText("One-time claim secret (save now)")}</p>
          ...
          {(onboardingSkillUrl || onboardingSkillPath || onboardingInstallPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{translateText("Paperclip skill bootstrap")}</p>
              ...
              {onboardingInstallPath && <p className="font-mono break-all">{translateText("Install to")} {onboardingInstallPath}</p>}
            </div>
          )}
          {(onboardingTextUrl || onboardingTextPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{translateText("Agent-readable onboarding text")}</p>
          ...
          {diagnostics.length > 0 && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{translateText("Connectivity diagnostics")}</p>
    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo" ? translateText("Bootstrap your Paperclip instance") : translateText("Join this Paperclip company")}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">{translateText("Invite expires")} {dateTime(invite.expiresAt)}.</p>
        ...
        {invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-5 flex gap-2">
            {availableJoinTypes.map((type) => (
              ...
              >
                {translateText("Join as")} {translateText(type)}
              </button>
            ))}
          </div>
        )}
        ...
        {joinType === "agent" && invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-4 space-y-3">
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{translateText("Agent name")}</span>
              <input
            ...
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{translateText("Adapter type")}</span>
              <select
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                value={adapterType}
              ...
              >
                {joinAdapterOptions.map((type) => (
                  <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                    {adapterLabels[type]}{!ENABLED_INVITE_ADAPTERS.has(type) ? ` (${translateText("Coming soon")})` : ""}
                  </option>
                ))}
              </select>
            </label>
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{translateText("Capabilities (optional)")}</span>
              <textarea
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                rows={4}
                value={capabilities}
        ...
        {requiresAuthForHuman && (
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            {translateText("Sign in or create an account before submitting a human join request.")}
            <div className="mt-2">
        ...
        >
          {acceptMutation.isPending
            ...
            : invite.inviteType === "bootstrap_ceo"
              ? translateText("Accept bootstrap invite")
              : translateText("Submit join request")}
        </Button>
      </div>
    </div>
  );
