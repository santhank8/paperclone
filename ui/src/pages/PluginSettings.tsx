...
export function PluginSettings() {
  const { t } = useTranslation(["pluginSettings", "common"]);
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("common:nav.plugins"), href: "/instance/settings/plugins" },
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{t("loadingDetails")}</div>;
  }
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t("noDescription");
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          ...
          items={[
            ...
            { value: "status", label: t("statusTab") },
          ]}
          value={activeTab}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t("noneLabel")}</span>
                      )}
                    </div>
                  </div>
                </div>
                  ...
                  {dashboardData ? (
                    <>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  ...
                                  <span className="text-xs">
                                    {t("consecutiveTotal", { consecutive: dashboardData.worker.consecutiveCrashes, total: dashboardData.worker.totalCrashes })}
                                  </span>
                                </div>
                      ...
                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        ...
                        {t("lastChecked")} {new Date(dashboardData.checkedAt).toLocaleTimeString()}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
              ...
              <Card>
                ...
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t("noPermissions")}</p>
                  )}
                </CardContent>
              </Card>
            </div>
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("failedSaveConfig") });
    },
  });
