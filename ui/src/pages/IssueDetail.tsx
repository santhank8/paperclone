...
const ACTION_LABEL_KEYS: Record<string, string> = {
  "issue.created": "issueDetail.action.createdIssue",
  "issue.updated": "issueDetail.action.updatedIssue",
  "issue.checked_out": "issueDetail.action.checkedOut",
  "issue.released": "issueDetail.action.released",
  "issue.comment_added": "issueDetail.action.addedComment",
  "issue.attachment_added": "issueDetail.action.addedAttachment",
  "issue.attachment_removed": "issueDetail.action.removedAttachment",
  "issue.document_created": "issueDetail.action.createdDocument",
  "issue.document_updated": "issueDetail.action.updatedDocument",
  "issue.document_deleted": "issueDetail.action.deletedDocument",
  "issue.deleted": "issueDetail.action.deletedIssue",
  "agent.created": "issueDetail.action.createdAgent",
  "agent.updated": "issueDetail.action.updatedAgent",
  "agent.paused": "issueDetail.action.pausedAgent",
  "agent.resumed": "issueDetail.action.resumedAgent",
  "agent.terminated": "issueDetail.action.terminatedAgent",
  "heartbeat.invoked": "issueDetail.action.invokedHeartbeat",
  "heartbeat.cancelled": "issueDetail.action.cancelledHeartbeat",
  "approval.created": "issueDetail.action.requestedApproval",
  "approval.approved": "issueDetail.action.approved",
  "approval.rejected": "issueDetail.action.rejected",
};

function actionLabel(action: string): string {
  const key = ACTION_LABEL_KEYS[action];
  return key ? i18n.t(key) : action.replace(/[._]/g, " ");
}
...
function formatAction(action: string, details?: Record<string, unknown> | null): string {
  if (action === "issue.updated" && details) {
    ...
    if (details.status !== undefined) {
      ...
      parts.push(
        from
          ? i18n.t("issueDetail.humanize.changedStatus", { from: humanizeValue(from), to: humanizeValue(details.status) })
          : i18n.t("issueDetail.humanize.changedStatus", { from: "", to: humanizeValue(details.status) })
      );
    }
    if (details.priority !== undefined) {
      ...
      parts.push(
        from
          ? i18n.t("issueDetail.humanize.changedPriority", { from: humanizeValue(from), to: humanizeValue(details.priority) })
          : i18n.t("issueDetail.humanize.changedPriority", { from: "", to: humanizeValue(details.priority) })
      );
    }
    if (details.assigneeAgentId !== undefined || details.assigneeUserId !== undefined) {
      parts.push(
        details.assigneeAgentId || details.assigneeUserId
          ? i18n.t("issueDetail.humanize.assigned")
          : i18n.t("issueDetail.humanize.unassigned"),
      );
    }
    if (details.title !== undefined) parts.push(i18n.t("issueDetail.humanize.updatedTitle"));
    if (details.description !== undefined) parts.push(i18n.t("issueDetail.humanize.updatedDescription"));
  ...
  if (
    ...
    return `${actionLabel(action)} ${key}${title}`;
  }
...
function ActorIdentity({ evt, agentMap }: { evt: ActivityEvent; agentMap: Map<string, Agent> }) {
  ...
  if (evt.actorType === "system") return <Identity name={i18n.t("issueDetail.identity.system")} size="sm" />;
  if (evt.actorType === "user") return <Identity name={i18n.t("issueDetail.identity.board")} size="sm" />;
  return <Identity name={id || i18n.t("issueDetail.identity.unknown")} size="sm" />;
}
...
export function IssueDetail() {
  ...
  const commentReassignOptions = useMemo(() => {
    ...
    if (currentUserId) {
      options.push({ id: `user:${currentUserId}`, label: t("issueDetail.identity.me") });
    }
    return options;
  ...
  const uploadAttachment = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) throw new Error(t("issueDetail.noCompany"));
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        <TabsList variant="line" className="w-full justify-start gap-1">
          <TabsTrigger value="comments" className="gap-1.5">
            ...
            {t("issueDetail.tab.comments")}
          </TabsTrigger>
          <TabsTrigger value="subissues" className="gap-1.5">
            <ListTree className="h-3.5 w-3.5" />
            {t("issueDetail.tab.subIssues")}
          </TabsTrigger>
      ...
      <Sheet open={mobilePropsOpen} onOpenChange={setMobilePropsOpen}>
        <SheetContent side="bottom" className="max-h-[85dvh] pb-[env(safe-area-inset-bottom)]">
          <SheetHeader>
            <SheetTitle className="text-sm">{t("issueDetail.propertiesDrawer")}</SheetTitle>
          </SheetHeader>
