  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("page.selectCompany")}</p>
      </div>
    );
  }

  if (!contributions) {
    return <div className="text-sm text-muted-foreground">{t("page.loading")}</div>;
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {t("page.multipleRoutes")} <code>{pluginRoutePath}</code>. Use the plugin-id route until the conflict is resolved.
        </div>
      );
    }
  }
