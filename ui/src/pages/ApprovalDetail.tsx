...
export function ApprovalDetail() {
  ...
  if (!approval) return <p className="text-sm text-muted-foreground">{t("pages.approvalDetail.notFound")}</p>;
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("pages.approvalDetail.reviewLinkedIssues")
              : t("pages.approvalDetail.reviewLinkedIssue"),
          to: `/issues/${primaryLinkedIssue.identifier ?? primaryLinkedIssue.id}`,
        }
      : linkedAgentId
        ? {
            label: t("pages.approvalDetail.openHiredAgent"),
            to: `/agents/${linkedAgentId}`,
          }
        : {
            label: t("pages.approvalDetail.backToApprovals"),
            to: "/approvals",
          };
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("pages.approvalDetail.approvalConfirmed")}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("pages.approvalDetail.agentNotified")}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          {approval.requestedByAgentId && (
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground text-xs">{t("pages.approvalDetail.requestedBy")}</span>
              <Identity
          ...
          >
            ...
            {t("pages.approvalDetail.seeFullRequest")}
          </button>
          {showRawPayload && (
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("pages.approvalDetail.decisionNote")} {approval.decisionNote}</p>
          )}
        </div>
        {error && <p className="text-sm text-destructive">{error}</p>}
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">{t("pages.approvalDetail.linkedIssues")}</p>
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t("pages.approvalDetail.linkedIssuesNote")}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          {isActionable && !isBudgetApproval && (
            <>
              ...
              >
                {t("pages.approvalDetail.approve")}
              </Button>
              <Button
                variant="destructive"
                size="sm"
              ...
              >
                {t("pages.approvalDetail.reject")}
              </Button>
            </>
          )}
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("pages.approvalDetail.budgetStopNote")} <Link to="/costs" className="underline underline-offset-2">/costs</Link>.
            </p>
          )}
          {approval.status === "pending" && (
            ...
            >
              {t("pages.approvalDetail.requestRevision")}
            </Button>
          )}
          {approval.status === "revision_requested" && (
            ...
            >
              {t("pages.approvalDetail.markResubmitted")}
            </Button>
          )}
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("pages.approvalDetail.confirmDeleteAgent"))) return;
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("pages.approvalDetail.comments", { count: comments?.length ?? 0 })}</h3>
        ...
        <Textarea
          ...
          placeholder={t("pages.approvalDetail.addComment")}
          rows={3}
        />
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? t("pages.approvalDetail.posting") : t("pages.approvalDetail.postComment")}
          </Button>
        </div>
      </div>
    </div>
