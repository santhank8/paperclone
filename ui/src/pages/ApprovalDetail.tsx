...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("approvals.title") },
    ]);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t("approvalDetail.linkedIssuesFollowup")}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("approvalDetail.resolveBudgetPrefix")} <Link to="/costs" className="underline underline-offset-2">/costs</Link>{t("approvalDetail.resolveBudgetSuffix")}
            </p>
          )}
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("approvalDetail.commentsCount", { count: comments?.length ?? 0 })}</h3>
            ...
            <div key={comment.id} className="border border-border/60 rounded-md p-3">
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  ...
                  <Identity name={t("approvalDetail.boardAuthor")} size="sm" />
                )}
