  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("page.goals.properties.status")}>
          {onUpdate ? (
            <PickerButton
              current={goal.status}
        ...
        <PropertyRow label={t("page.goals.properties.level")}>
          {onUpdate ? (
            <PickerButton
              current={goal.level}
              options={GOAL_LEVELS}
        ...
        <PropertyRow label={t("page.goals.properties.owner")}>
          {ownerAgent ? (
            <Link
            ...
            <span className="text-sm text-muted-foreground">{t("page.goals.properties.none")}</span>
          )}
        </PropertyRow>

        {goal.parentId && (
          <PropertyRow label={t("page.goals.properties.parent_goal")}>
            <Link
      ...
      <div className="space-y-1">
        <PropertyRow label={t("page.goals.properties.created")}>
          <span className="text-sm">{formatDate(goal.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={t("page.goals.properties.updated")}>
