...
const ISSUE_STATUS_VALUES = ["backlog", "todo", "in_progress", "in_review", "done"] as const;
...
export function NewIssueDialog() {
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          title: t("newIssue.createdWithWarnings", { issue: issueRef }),
          body: t("newIssue.uploadWarningsBody", {
            count: failures.length,
            noun: failures.length === 1 ? t("newIssue.file") : t("newIssue.files"),
          }),
          tone: "warn",
          action: prefix
            ? { label: t("newIssue.openIssue", { issue: issueRef }), href: `/${prefix}/issues/${issueRef}` }
            : undefined,
        });
      }
      clearDraft();
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!effectiveCompanyId) throw new Error(t("newIssue.noCompanySelected"));
  ...
  const statusOptions = useMemo(
    () =>
      ISSUE_STATUS_VALUES.map((value) => ({
        ...
        label:
          value === "backlog"
            ? t("newIssue.options.backlog")
            : value === "todo"
              ? t("newIssue.options.todo")
              : value === "in_progress"
                ? t("newIssue.options.inProgress")
                : value === "in_review"
                  ? t("newIssue.options.inReview")
                  : t("newIssue.options.done"),
  ...
  const priorityOptions = useMemo(
    () =>
      ISSUE_PRIORITY_VALUES.map((entry) => ({
        ...entry,
        label:
          entry.value === "critical"
            ? t("newIssue.options.critical")
            : entry.value === "high"
              ? t("newIssue.options.high")
              : entry.value === "medium"
                ? t("newIssue.options.medium")
                : t("newIssue.options.low"),
      })),
    [t],
  );
  const executionWorkspaceModeOptions = useMemo(
    () =>
      EXECUTION_WORKSPACE_MODE_VALUES.map((value) => ({
        value,
        label:
          value === "shared_workspace"
            ? t("newIssue.options.projectDefault")
            : value === "isolated_workspace"
              ? t("newIssue.options.newIsolatedWorkspace")
              : t("newIssue.options.reuseExistingWorkspace"),
      })),
    [t],
  );
  ...
  const assigneeOptionsTitle =
    assigneeAdapterType === "claude_local"
      ...
      : assigneeAdapterType === "codex_local"
        ...
        : t("newIssue.agentOptions.generic");
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("newIssue.failedCreate");
              ...
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("sharedUi.assignee")}</span>
                  )
                }
                renderOption={(option) => {
          ...
          <div className="px-4 py-3 shrink-0 space-y-2">
            <div className="space-y-1.5">
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  {t("newIssue.reuseWorkspaceSummary", {
              ...
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  ...
                  <InlineEntitySelector
                    ...
                    emptyMessage={t("onboarding.noModelsDiscovered")}
                    ...
                    {thinkingEffortOptions.map((option) => (
                      <button
                        ...
                        className={cn(
                          ...
                          assigneeThinkingEffort === option && "bg-accent"
                        )}
                        onClick={() => setAssigneeThinkingEffort(option)}
                      >
                        {option === ""
                          ? t("newIssue.options.default")
                          : option === "minimal"
                            ? t("newIssue.options.minimal")
                            : option === "low"
                              ? t("newIssue.options.low")
                              : option === "medium"
                                ? t("newIssue.options.medium")
                      ...
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 text-[11px] text-muted-foreground">
                            {file.file.type || t("newIssue.binaryFile")} • {formatFileSize(file.file)}
                          </div>
                        </div>
                        <Button
                          variant="ghost"
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? t("newIssue.creatingIssue") : t("newIssue.createIssue")}</span>
              </span>
            </Button>
          </div>
        </div>
