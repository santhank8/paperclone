...
const statuses = [
  { value: "backlog", labelKey: "Backlog", color: issueStatusText.backlog ?? issueStatusTextDefault },
  { value: "todo", labelKey: "Todo", color: issueStatusText.todo ?? issueStatusTextDefault },
  { value: "in_progress", labelKey: "In Progress", color: issueStatusText.in_progress ?? issueStatusTextDefault },
  { value: "in_review", labelKey: "In Review", color: issueStatusText.in_review ?? issueStatusTextDefault },
  { value: "done", labelKey: "Done", color: issueStatusText.done ?? issueStatusTextDefault },
];

const priorities = [
  { value: "critical", labelKey: "Critical", icon: AlertTriangle, color: priorityColor.critical ?? priorityColorDefault },
  { value: "high", labelKey: "High", icon: ArrowUp, color: priorityColor.high ?? priorityColorDefault },
  { value: "medium", labelKey: "Medium", icon: Minus, color: priorityColor.medium ?? priorityColorDefault },
  { value: "low", labelKey: "Low", icon: ArrowDown, color: priorityColor.low ?? priorityColorDefault },
];

const EXECUTION_WORKSPACE_MODES = [
  { value: "shared_workspace", labelKey: "Project default" },
  { value: "isolated_workspace", labelKey: "New isolated workspace" },
  { value: "reuse_existing", labelKey: "Reuse existing workspace" },
] as const;
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          title: `${t("Issue created")}: ${issueRef} ${t("with upload warnings")}`,
          body: `${failures.length} ${t("staged files could not be added.")}`,
          tone: "warn",
          action: prefix
