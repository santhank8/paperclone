          ...
          <div className="px-4 py-3 shrink-0 space-y-2">
            <div className="space-y-1.5">
              ...
              >
                {EXECUTION_WORKSPACE_MODES.map((option) => (
                  <option key={option.value} value={option.value}>
                    {t(`newIssue.workspaceModeLabels.${option.value}`)}
                  </option>
                ))}
              </select>
              {executionWorkspaceMode === "reuse_existing" && (
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  {t("newIssue.reusingWorkspace", { name: selectedReusableExecutionWorkspace.name, location: selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? selectedReusableExecutionWorkspace.id.slice(0, 8) })}
                </div>
              )}
            </div>
          </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {thinkingEffortOptions.map((option) => (
                      ...
                      >
                        {t(`newIssue.thinkingEffortLabels.${option.value || "default"}`)}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <Popover open={statusOpen} onOpenChange={setStatusOpen}>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                ...
                {t(`newIssue.statusLabels.${currentStatus.value}`)}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-36 p-1" align="start">
              {statuses.map((s) => (
                ...
                >
                  ...
                  {t(`newIssue.statusLabels.${s.value}`)}
                </button>
              ))}
            </PopoverContent>
          </Popover>
              ...
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                {currentPriority ? (
                  <>
                    ...
                    {t(`newIssue.priorityLabels.${currentPriority.value}`)}
                  </>
                ) : (
                  <>
                    <Minus className="h-3 w-3 text-muted-foreground" />
                    {t("newIssue.priorityLabels.none")}
                  </>
                )}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-36 p-1" align="start">
              {priorities.map((p) => (
                ...
                >
                  ...
                  {t(`newIssue.priorityLabels.${p.value}`)}
                </button>
              ))}
            </PopoverContent>
          </Popover>
