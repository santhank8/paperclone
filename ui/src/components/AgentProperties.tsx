  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <PropertyRow label={t("properties.role", { defaultValue: "Role" })}>
          <span className="text-sm">{roleLabels[agent.role] ?? agent.role}</span>
        </PropertyRow>
        {agent.title && (
          <PropertyRow label={t("properties.title", { defaultValue: "Title" })}>
            <span className="text-sm">{agent.title}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("properties.adapter", { defaultValue: "Adapter" })}>
      ...
      <div className="space-y-1">
        {(runtimeState?.sessionDisplayId ?? runtimeState?.sessionId) && (
          <PropertyRow label={t("properties.session", { defaultValue: "Session" })}>
        ...
        {runtimeState?.lastError && (
          <PropertyRow label={t("properties.lastError", { defaultValue: "Last error" })}>
            <span className="text-xs text-red-600 dark:text-red-400 break-words min-w-0">{runtimeState.lastError}</span>
          </PropertyRow>
        )}
        {agent.lastHeartbeatAt && (
          <PropertyRow label={t("properties.lastHeartbeat", { defaultValue: "Last Heartbeat" })}>
            <span className="text-sm">{formatDate(agent.lastHeartbeatAt)}</span>
          </PropertyRow>
        )}
        {agent.reportsTo && (
          <PropertyRow label={t("properties.reportsTo", { defaultValue: "Reports To" })}>
            {reportsToAgent ? (
