  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {t(`projectProperties.statusLabels.${status}`, { defaultValue: status.replace("_", " ") })}
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-40 p-1" align="start">
        {PROJECT_STATUSES.map((s) => (
          ...
          >
            {t(`projectProperties.statusLabels.${s.value}`, { defaultValue: s.label })}
          </Button>
        ))}
      </PopoverContent>
    </Popover>
      ...
      {archivePending ? (
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {isArchive
              ? t("projectProperties.archiveConfirmPrompt", { name: project.name })
              : t("projectProperties.unarchiveConfirmPrompt", { name: project.name })}
          </span>
          <Button
            size="sm"
            variant="destructive"
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(t("projectProperties.localPathError"));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!isGitHubRepoUrl(repoUrl)) {
      setWorkspaceError(t("projectProperties.repoUrlError"));
      return;
    }
    setWorkspaceError(null);
          ...
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <Tooltip>
              ...
              <TooltipContent side="top">
                {t("projectProperties.codebaseHelpTooltip")}
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {t("projectProperties.legacyWorkspacesNote")}
              </div>
            )}
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="top">
                    {t("projectProperties.executionWorkspacesHelpTooltip")}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {t("projectProperties.enableIsolatedCheckoutsDescription")}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <button
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        ...
                        <div className="text-[11px] text-muted-foreground">
                          {t("projectProperties.newIssuesDefaultIsolatedDescription")}
                        </div>
                      </div>
                      <button
                        className={cn(
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {t("projectProperties.provisionTeardownDescription")}
                        </p>
                      </div>
                    ) : null}
                  </div>
