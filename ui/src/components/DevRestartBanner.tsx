        ...
        <div className="min-w-0">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-amber-900/80 dark:text-amber-100/75">
            {sample.length > 0 ? (
              <span>
                已更改: {sample.join(", ")}
                {devServer.changedPathCount > sample.length ? ` 还有 ${devServer.changedPathCount - sample.length} 个` : ""}
              </span>
            ) : null}
            {devServer.pendingMigrations.length > 0 ? (
              <span>
                待处理迁移: {devServer.pendingMigrations.slice(0, 2).join(", ")}
                {devServer.pendingMigrations.length > 2 ? ` 还有 ${devServer.pendingMigrations.length - 2} 个` : ""}
              </span>
            ) : null}
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-2 text-xs font-medium">
          {devServer.waitingForIdle ? (
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-900/10 px-3 py-1.5 dark:bg-amber-100/10">
              ...
              <span>正在等待 {devServer.activeRunCount} 个运行中的任务完成</span>
            </div>
          ) : devServer.autoRestartEnabled ? (
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-900/10 px-3 py-1.5 dark:bg-amber-100/10">
            ...
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-900/10 px-3 py-1.5 dark:bg-amber-100/10">
              <RotateCcw className="h-3.5 w-3.5" />
              <span>在可以安全中断当前工作后，请重启 <code>pnpm dev:once</code></span>
            </div>
          )}
        </div>
      </div>
