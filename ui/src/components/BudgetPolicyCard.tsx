...
export function BudgetPolicyCard({
  ...
  const scopeTypeLabel =
    summary.scopeType === "project"
      ? t("components.budgetPolicyCard.scope.project")
      : t("components.budgetPolicyCard.scope.company");
  ...
  const windowLabel = (windowKind: BudgetPolicySummary["windowKind"]) =>
    windowKind === "lifetime"
      ? t("components.budgetPolicyCard.window_lifetime")
      : t("components.budgetPolicyCard.window_monthly_utc");

  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("components.budgetPolicyCard.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0
            ? t("components.budgetPolicyCard.percent_of_limit", { percent: summary.utilizationPercent })
            : t("components.budgetPolicyCard.no_cap_configured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("components.budgetPolicyCard.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("components.budgetPolicyCard.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("components.budgetPolicyCard.soft_alert_at", { percent: summary.warnPercent })}
          {summary.paused && summary.pauseReason ? ` - ${summary.pauseReason} ${t("components.budgetPolicyCard.pause")}` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("components.budgetPolicyCard.remaining")}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("components.budgetPolicyCard.unlimited")}</span>
      </div>
  ...
  const pausedPane = summary.paused ? (
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("components.budgetPolicyCard.project_paused_message")
          : t("components.budgetPolicyCard.scope_paused_message")}
      </div>
    </div>
  ) : null;
    ...
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      <div className="min-w-0 flex-1">
        <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {t("components.budgetPolicyCard.budget_usd")}
        </label>
        <Input
          value={draftBudget}
          ...
          className="mt-2"
          inputMode="decimal"
          placeholder={t("components.budgetPolicyCard.amount_placeholder")}
        />
      </div>
      <Button
        onClick={() => {
      ...
      >
        {isSaving
          ? t("components.budgetPolicyCard.saving")
          : summary.amount > 0
            ? t("components.budgetPolicyCard.update_budget")
            : t("components.budgetPolicyCard.set_budget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused
              ? t("components.budgetPolicyCard.status.paused")
              : summary.status === "warning"
                ? t("components.budgetPolicyCard.status.warning")
                : summary.status === "hard_stop"
                  ? t("components.budgetPolicyCard.status.hard_stop")
                  : t("components.budgetPolicyCard.status.healthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("components.budgetPolicyCard.invalid_amount")}</p>
        ) : null}
      </div>
    );
  }
