...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">已使用</div>
      ...
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.warnPercent}% 时软告警{summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} 暂停` : ""}
        </div>
      </div>
    </div>
  ) : (
