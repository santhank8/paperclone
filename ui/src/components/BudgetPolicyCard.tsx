    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("budgetPolicyCard.ofLimit", { percent: summary.utilizationPercent }) : t("budgetPolicyCard.noCapConfigured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budgetPolicyCard.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("budgetPolicyCard.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("budgetPolicyCard.softAlertAt", { percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} ${t("budgetPolicyCard.pause")}` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("budgetPolicyCard.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("budgetPolicyCard.projectPaused")
          : t("budgetPolicyCard.scopePaused")}
      </div>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <div className="mt-2 text-sm text-muted-foreground">{summary.windowKind === "lifetime" ? t("budgetPolicyCard.lifetimeBudget") : t("budgetPolicyCard.monthlyBudget")}</div>
          </div>
          <div
            className={cn(
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("budgetPolicyCard.invalidAmount")}</p>
        ) : null}
      </div>
    );
  }
      ...
      <CardHeader className={cn("gap-3", compact ? "px-4 pt-4 pb-2" : "px-5 pt-5 pb-3")}>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1">{summary.windowKind === "lifetime" ? t("budgetPolicyCard.lifetimeBudget") : t("budgetPolicyCard.monthlyBudget")}</CardDescription>
          </div>
