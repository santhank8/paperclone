    ...
    >
      ...
      >
        ...
        <div className="px-4 pb-3 space-y-3 border-t border-border">
          <div className="pt-3">
            <p className="text-sm font-medium">{t('projects.workspaceQuestion')}</p>
            <p className="text-xs text-muted-foreground">{t('projects.workspaceHint')}</p>
          </div>
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                {t('projects.localFolder')}
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t('projects.localFolderHint')}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t('projects.githubRepoHint')}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t('projects.bothHint')}</p>
            </button>
          </div>

          {(workspaceSetup === "local" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t('projects.localFolderLabel')}</label>
          ...
          {(workspaceSetup === "repo" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t('projects.githubRepoLabel')}</label>
              <input
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t('projects.addGoal') : t('projects.goal')}
              </button>
            </PopoverTrigger>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t('projects.failedToCreate')}</p>
          ) : (
            <span />
          )}
          <Button
