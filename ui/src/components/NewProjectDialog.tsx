...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (localRequired && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("new.localFolderError"));
      return;
    }
    if (repoRequired && !isGitHubRepoUrl(repoUrl)) {
      setWorkspaceError(t("new.repoUrlError"));
      return;
    }
      ...
      >
        ...
        <div className="px-4 pb-3 space-y-3 border-t border-border">
          ...
          {(workspaceSetup === "local" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t("new.localFolder")}</label>
          ...
          {(workspaceSetup === "repo" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t("new.githubRepo")}</label>
              <input
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("new.createFailed", { defaultValue: "Failed to create project." })}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? t("new.creating") : t("new.create")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
