    ...
    <div>
      ...
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">暂无最近的 Agent 运行记录。</p>
        </div>
      ) : (
    ...
    <div className={cn(
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive ? "正在直播" : run.finishedAt ? `完成于 ${relativeTime(run.finishedAt)}` : `开始于 ${relativeTime(run.createdAt)}`}</span>
            </div>
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <RunTranscriptView
          ...
          streaming={isActive}
          collapseStdout
          emptyMessage={hasOutput ? "等待运行记录解析..." : isActive ? "等待输出..." : "未捕获运行记录。"}
        />
      </div>
    </div>
  );
