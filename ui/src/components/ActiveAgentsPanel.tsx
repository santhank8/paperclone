  ...
  return (
    <div>
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
        {t("activeAgentsPanel.agents")}
      </h3>
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">{t("activeAgentsPanel.noRecentRuns")}</p>
        </div>
      ) : (
    ...
    <div className={cn(
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <RunTranscriptView
          ...
          emptyMessage={hasOutput ? t("activeAgentsPanel.waitingTranscript") : isActive ? t("activeAgentsPanel.waitingOutput") : t("activeAgentsPanel.noTranscript")}
        />
      </div>
    </div>
  );
