    ...
    <div className={cn(
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive ? "实时运行中" : run.finishedAt ? `${relativeTime(run.finishedAt)}完成` : `${relativeTime(run.createdAt)}开始`}</span>
            </div>
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <RunTranscriptView
          ...
          emptyMessage={hasOutput ? "等待转录解析中..." : isActive ? "等待输出中..." : "未捕获到转录。"}
        />
      </div>
    </div>
  );
