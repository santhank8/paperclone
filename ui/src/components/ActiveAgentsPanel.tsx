    ...
    <div>
      ...
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">{t("activeAgents.none")}</p>
        </div>
      ) : (
    ...
    <div className={cn(
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>
                {isActive
                  ...
                  : run.finishedAt
                    ? t("activeAgents.finished", { time: relativeTime(run.finishedAt) })
                    : t("activeAgents.started", { time: relativeTime(run.createdAt) })}
              </span>
            </div>
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <RunTranscriptView
          ...
          emptyMessage={hasOutput
            ? t("activeAgents.waitingTranscriptParsing")
            : isActive
              ? t("activeAgents.waitingOutput")
              : t("activeAgents.noTranscriptCaptured")}
        />
      </div>
    </div>
  );
