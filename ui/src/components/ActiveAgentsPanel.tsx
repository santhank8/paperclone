    ...
    <div className={cn(
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>
                {isActive
                  ? t("dashboard.liveNow")
                  : run.finishedAt
                    ? t("dashboard.finishedAgo", { time: relativeTime(run.finishedAt) })
                    : t("dashboard.startedAgo", { time: relativeTime(run.createdAt) })}
              </span>
            </div>
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <RunTranscriptView
          ...
          emptyMessage={
            hasOutput
              ? t("dashboard.waitingForTranscriptParsing")
              : isActive
                ? t("dashboard.waitingForOutput")
                : t("dashboard.noTranscriptCaptured")
          }
        />
      </div>
    </div>
