...
export function IssuesList({
  ...
  const groupedContent = useMemo(() => {
    ...
    return Object.keys(groups).map((key) => ({
      key,
      label: key === "__unassigned" ? t("noAssignee", { ns: "common" }) : (agentName(key) ?? key.slice(0, 8)),
      items: groups[key]!,
    }));
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? t("filtersCount", { count: activeFilterCount }) : t("filter", { ns: "common" })}</span>
                {activeFilterCount > 0 && (
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-1">
                    <span className="text-xs text-muted-foreground">{t("sort.status")}</span>
                  ...
                  <div className="space-y-3">
                    {/* Priority */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">{t("sort.priority")}</span>
                      <div className="space-y-0.5">
                        {priorityOrder.map((p) => (
