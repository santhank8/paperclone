...
const statusI18nKeys: Record<string, string> = {
  ...
  in_review: "issues.inReview",
  blocked: "issues.blocked",
  done: "issues.done",
  cancelled: "issues.cancelled",
};
...
const quickFilterPresets = [
  { labelKey: "issues.filterAll", statuses: [] as string[] },
  { labelKey: "issues.filterActive", statuses: ["todo", "in_progress", "in_review", "blocked"] },
  { labelKey: "issues.filterBacklog", statuses: ["backlog"] },
  { labelKey: "issues.filterDone", statuses: ["done", "cancelled"] },
];
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "status") {
      ...
      return statusOrder
        ...
        .map((s) => ({ key: s, label: t(statusI18nKeys[s] ?? s), items: groups[s]! }));
    }
    if (viewState.groupBy === "priority") {
      const groups = groupBy(filtered, (i) => i.priority);
      return priorityOrder
        .filter((p) => groups[p]?.length)
        .map((p) => ({ key: p, label: t(priorityI18nKeys[p] ?? p), items: groups[p]! }));
    }
    // assignee
    const groups = groupBy(filtered, (i) => i.assigneeAgentId ?? "__unassigned");
    return Object.keys(groups).map((key) => ({
      key,
      label: key === "__unassigned" ? t('common.unassigned') : (agentName(key) ?? key.slice(0, 8)),
      items: groups[key]!,
    }));
        ...
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          <div className="relative w-48 sm:w-64 md:w-80">
            ...
            <Input
              ...
              aria-label={t('issues.searchIssuesLabel')}
            />
          </div>
        </div>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? t('issues.filtersCount', { count: activeFilterCount }) : t('common.filters')}</span>
                {activeFilterCount > 0 && (
            ...
            <PopoverContent align="end" className="w-[min(480px,calc(100vw-2rem))] p-0">
              <div className="p-3 space-y-3">
                ...
                <div className="space-y-1.5">
                  <span className="text-xs text-muted-foreground">{t('common.quickFilters')}</span>
                    ...
                    <div className="space-y-0.5">
                      {statusOrder.map((s) => (
                        <label key={s} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <StatusIcon status={s} />
                          <span className="text-sm">{t(statusI18nKeys[s] ?? s)}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                      ...
                      <div className="space-y-0.5">
                        {priorityOrder.map((p) => (
                          <label key={p} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            ...
                            <span className="text-sm">{t(priorityI18nKeys[p] ?? p)}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 shrink-0 ml-auto">
                    {liveIssueIds?.has(issue.id) && (
                      <span className="inline-flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-0.5 rounded-full bg-blue-500/10">
                        ...
                        <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400 hidden sm:inline">{t('common.live')}</span>
                      </span>
                    )}
