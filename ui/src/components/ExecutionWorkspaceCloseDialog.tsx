...
export function ExecutionWorkspaceCloseDialog({
  ...
  const tt = (key: string, vars?: Record<string, string | number>) => t(`executionWorkspaceClose.${key}`, vars);
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? tt("closeRetried") : tt("closed"),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: tt("failedClose"),
        body: error instanceof Error ? error.message : tt("unknownError"),
        tone: "error",
      });
    },
  });
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
          ...
          <DialogDescription className="break-words">
            <span
              dangerouslySetInnerHTML={{
                __html: tt("archiveDescription", { name: `<span class="font-medium text-foreground">${workspaceName}</span>` }),
              }}
            />
          </DialogDescription>
        </DialogHeader>
        ...
        {readinessQuery.isLoading ? (
          ...
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : tt("failedInspect")}
          </div>
        ) : readiness ? (
          <div className="space-y-4">
            ...
            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tt("blockingIssues")}</h3>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tt("blockingReasons")}</h3>
            ...
            {readiness.warnings.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tt("warnings")}</h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tt("gitStatus")}</h3>
                <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tt("branch")}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? tt("unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tt("baseRef")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? tt("notSet")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tt("mergedIntoBase")}</div>
                      <div>{readiness.git.isMergedIntoBase == null ? tt("unknown") : readiness.git.isMergedIntoBase ? tt("yes") : tt("no")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tt("aheadBehind")}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tt("dirtyTrackedFiles")}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tt("untrackedFiles")}</div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tt("otherLinkedIssues")}</h3>
                <div className="space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tt("attachedRuntimeServices")}</h3>
                <div className="space-y-2">
                  {readiness.runtimeServices.map((service) => (
                    <div key={service.id} className="rounded-xl border border-border bg-muted/20 px-4 py-3 text-sm">
                      ...
                      <div className="mt-1 break-words text-xs text-muted-foreground">
                        {service.url ?? service.command ?? service.cwd ?? tt("noAdditionalDetails")}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{tt("cleanupActions")}</h3>
              <div className="space-y-2">
            ...
            {readiness.git?.repoRoot ? (
              <div className="break-words text-xs text-muted-foreground">
                {tt("repoRoot")}: <span className="font-mono break-all">{readiness.git.repoRoot}</span>
                {readiness.git.workspacePath ? (
                  <>
                    {" · "}{tt("workspacePath")}: <span className="font-mono break-all">{readiness.git.workspacePath}</span>
                  </>
                ) : null}
              </div>
            ) : null}
            ...
            <div className="text-xs text-muted-foreground">
              {tt("lastChecked")} {formatDateTime(new Date())}
            </div>
          </div>
        ) : null}
