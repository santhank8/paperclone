            ...
            <Field label={t("agentConfig.workingDirectoryDeprecated")} hint={help.cwd}>
              <div className="flex items-center gap-2 rounded-md border border-border px-2.5 py-1.5">
                ...
                <DraftInput
                  ...
                  placeholder={t("agentConfig.cwdPlaceholder")}
                />
                <ChoosePathButton />
              </div>
            </Field>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              {!isCreate && (
                <>
                  ...
                  <Field label={t("agentConfig.interruptGracePeriod")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
  ...
  return (
    <div className="space-y-1.5">
          ...
          <div key={i} className="flex items-center gap-1.5">
            ...
            {row.source === "secret" ? (
              <>
                ...
                >
                  <option value="">{t("agentConfig.env.selectSecret")}</option>
                  {secrets.map((secret) => (
                ...
                <button
                  ...
                  title={t("agentConfig.env.createSecretTitle")}
                >
                  {t("agentConfig.env.new")}
                </button>
              </>
            ) : (
              <>
                <input
                  ...
                  placeholder={t("agentConfig.env.valuePlaceholder")}
                ...
                <button
                  ...
                  onClick={() => sealRow(i)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("agentConfig.env.sealTitle")}
                >
                  {t("agentConfig.env.seal")}
                </button>
              </>
            )}
            {!isTrailing ? (
      ...
      <p className="text-[11px] text-muted-foreground/60">
        {t("agentConfig.env.autoInjected")}
      </p>
    </div>
  );
}
        ...
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value || (allowDefault ? t("agentConfig.modelDefault") : required ? t("agentConfig.selectModelRequired") : t("agentConfig.selectModel"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
