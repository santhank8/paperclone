...
function useThinkingEffortOptions() {
  const { t } = useTranslation();
  return useMemo(() => ({
    codex: [
      { id: "", label: t("agentConfigForm.effortAuto") },
      { id: "minimal", label: t("agentConfigForm.effortMinimal") },
      { id: "low", label: t("agentConfigForm.effortLow") },
      { id: "medium", label: t("agentConfigForm.effortMedium") },
      { id: "high", label: t("agentConfigForm.effortHigh") },
    ],
    openCode: [
      { id: "", label: t("agentConfigForm.effortAuto") },
      { id: "minimal", label: t("agentConfigForm.effortMinimal") },
      ...
      { id: "medium", label: t("agentConfigForm.effortMedium") },
      { id: "high", label: t("agentConfigForm.effortHigh") },
      { id: "max", label: t("agentConfigForm.effortMax") },
    ],
    cursor: [
      { id: "", label: t("agentConfigForm.effortAuto") },
      { id: "plan", label: t("agentConfigForm.cursorPlan") },
      { id: "ask", label: t("agentConfigForm.cursorAsk") },
    ],
    claude: [
      { id: "", label: t("agentConfigForm.effortAuto") },
      { id: "low", label: t("agentConfigForm.effortLow") },
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const effortOptions = useThinkingEffortOptions();
  ...
  const thinkingEffortOptions =
    adapterType === "codex_local"
      ? effortOptions.codex
      : adapterType === "cursor"
        ? effortOptions.cursor
        : adapterType === "opencode_local"
          ? effortOptions.openCode
          : effortOptions.claude;
  ...
  return (
    <div className={cn("relative", cards && "space-y-6")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agentConfigForm.name")} hint={help.name}>
              <DraftInput
                ...
                placeholder={t("agentConfigForm.agentNamePlaceholder")}
              />
            </Field>
            <Field label={t("agentConfigForm.title")} hint={help.title}>
              <DraftInput
                ...
                immediate
                className={inputClass}
                placeholder={t("agentConfigForm.titlePlaceholder")}
              />
            </Field>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                {t("agentConfigForm.promptTemplateWarningCreate")}
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentConfigForm.permissionsAndConfiguration")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentConfigForm.permissionsAndConfiguration")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              {fetchedModelsError && (
                <p className="text-xs text-destructive">
                  {fetchedModelsError instanceof Error
                    ...
                    : t("agentConfigForm.failedToLoadModels")}
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("agentConfigForm.codexMinimalWarning")}
                      </p>
                    )}
                </>
              )}
                ...
                <>
                  <Field label={t("agentConfigForm.bootstrapPromptLegacy")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      ...
                      placeholder={t("agentConfigForm.bootstrapPromptPlaceholder")}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("agentConfigForm.bootstrapPromptWarning")}
                  </div>
                </>
              )}
              {adapterType === "claude_local" && (
              ...
              {!isCreate && (
                <>
                  ...
                  <Field label={t("agentConfigForm.interruptGracePeriod")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
...
function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const { t } = useTranslation();
  const statusLabel =
    result.status === "pass" ? t("agentConfigForm.statusPassed") : result.status === "warn" ? t("agentConfigForm.statusWarnings") : t("agentConfigForm.statusFailed");
  const statusClass =
    result.status === "pass"
...
function EnvVarEditor({
  ...
  async function sealRow(i: number) {
    ...
    const name = window.prompt(t("agentConfigForm.secretNamePrompt"), suggested)?.trim();
    if (!name) return;
    ...
    try {
      ...
      setSealError(err instanceof Error ? err.message : t("agentConfigForm.failedToCreateSecret"));
    }
  }
  ...
  return (
    <div className="space-y-1.5">
        ...
        return (
          <div key={i} className="flex items-center gap-1.5">
            <input
              ...
              placeholder={t("agentConfigForm.envKey")}
              value={row.key}
            ...
            >
              <option value="plain">{t("agentConfigForm.envPlain")}</option>
              <option value="secret">{t("agentConfigForm.envSecret")}</option>
            </select>
            {row.source === "secret" ? (
              <>
                <select
                ...
                <button
                  ...
                  title={t("agentConfigForm.createSecretFromValue")}
                >
              ...
              <>
                <input
                  ...
                  placeholder={t("agentConfigForm.envValue")}
                ...
                <button
                  ...
                  onClick={() => sealRow(i)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("agentConfigForm.sealTitle")}
                >
      ...
      <p className="text-[11px] text-muted-foreground/60">
        {t("agentConfigForm.paperclipVarsNote")}
      </p>
    </div>
  );
}
        ...
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value || (allowDefault ? t("agentConfigForm.default") : required ? t("agentConfigForm.selectModelRequired") : t("agentConfigForm.selectModel"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
  ...
  return (
    <Field label={t("agentConfigForm.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? t("agentConfigForm.effortAuto")}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
