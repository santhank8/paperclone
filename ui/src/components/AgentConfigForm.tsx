    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            ...
            >
              {!isCreate && props.isSaving ? t("configForm.saving") : t("configForm.save")}
            </Button>
          </div>
        </div>
      )}
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("configForm.name")} hint={help.name}>
              <DraftInput
                ...
                placeholder={t("configForm.agentNamePlaceholder")}
              />
            </Field>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          ...
          >
            {testEnvironment.isPending ? t("configForm.testing") : t("configForm.testEnvironment")}
          </Button>
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {/* Working directory */}
          {isLocal && (
            <Field label={t("configForm.workingDirectory")} hint={help.cwd}>
              <div className="flex items-center gap-2 rounded-md border border-border px-2.5 py-1.5">
                ...
                <DraftInput
                  ...
                  placeholder={t("configForm.workingDirectoryPlaceholder")}
                />
                <ChoosePathButton />
              </div>
            </Field>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                {t("configForm.promptTemplateWarningCreate")}
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("configForm.permissionsConfig")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("configForm.permissionsConfig")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("configForm.codexMinimalThinkingWarning")}
                      </p>
                    )}
                </>
              )}
              ...
              {!isCreate && (
                <>
                  <Field label={t("configForm.timeout")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <div className="space-y-3">
              ...
              />
              <Field label={t("configForm.cooldown")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
...
function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const { t } = useTranslation("agents");
  const statusLabel =
    result.status === "pass" ? t("configForm.envTest.passed") : result.status === "warn" ? t("configForm.envTest.warnings") : t("configForm.envTest.failed");
  const statusClass =
    result.status === "pass"
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{t("configForm.envTest.hint")} {check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-1.5">
        ...
        return (
          <div key={i} className="flex items-center gap-1.5">
            <input
              ...
              placeholder={t("configForm.envVarKeyPlaceholder")}
              value={row.key}
            ...
            >
              <option value="plain">{t("configForm.plain")}</option>
              <option value="secret">{t("configForm.secret")}</option>
            </select>
            {row.source === "secret" ? (
              <>
                <select
                ...
                >
                  <option value="">{t("configForm.selectSecret")}</option>
                  {secrets.map((secret) => (
                ...
                <button
                  ...
                  title={t("configForm.newSecretTooltip")}
                >
                  {t("configForm.newSecret")}
                </button>
              </>
            ) : (
              <>
                ...
                <button
                  ...
                  onClick={() => sealRow(i)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("configForm.sealSecretTooltip")}
                >
                  {t("configForm.sealSecret")}
                </button>
              </>
            )}
            {!isTrailing ? (
      ...
      <p className="text-[11px] text-muted-foreground/60">
        {t("configForm.envVarPlaceholder")}
      </p>
    </div>
  );
}
...
function ModelDropdown({
  ...
  return (
    <Field label={t("configForm.model")} hint={help.model}>
      <Popover
        open={open}
      ...
      >
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value || (allowDefault ? t("configForm.modelDefault") : required ? t("configForm.modelSelectRequired") : t("configForm.modelSelect"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <input
            ...
            placeholder={t("configForm.modelSearchPlaceholder")}
            value={modelSearch}
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {filteredModels.length === 0 && (
              <p className="px-2 py-1.5 text-xs text-muted-foreground">{t("configForm.noModelsFound")}</p>
            )}
          </div>
        </PopoverContent>
      </Popover>
  ...
  return (
    <Field label={t("configForm.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? t("configForm.thinkingEffortAuto")}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
