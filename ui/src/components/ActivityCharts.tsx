...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  const priorityLabels: Record<string, string> = {
    critical: t.issues.priorityUrgent,
    high: t.issues.priorityHigh,
    medium: t.issues.priorityMedium,
    low: t.issues.priorityLow,
  };
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map(p => ({ color: priorityColors[p], label: priorityLabels[p] ?? p.charAt(0).toUpperCase() + p.slice(1) }))} />
    </div>
  );
}
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "#6b7280", label: t.issueStatuses[s as keyof typeof t.issueStatuses] ?? s }))} />
    </div>
  );
}
