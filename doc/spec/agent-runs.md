...
This spec defines how PrivateClip actually runs agents while staying runtime-agnostic.
...
1. PrivateClip is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, PrivateClip manages lifecycle, and on exit PrivateClip parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared PrivateClip skill text block)
