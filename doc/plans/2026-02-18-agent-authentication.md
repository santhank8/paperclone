...
Agents need API keys to authenticate with PrivateClip. The current approach
...
**Trust model:** The adapter process runs on the same machine as the PrivateClip
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** PrivateClip generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a PrivateClip CLI
...
managed by the PrivateClip server itself.
...
1. A company admin (user or agent) generates an **invite URL** from PrivateClip.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The PrivateClip SKILL.md (or a link to it)
   - What information PrivateClip needs from the agent (e.g. webhook URL, adapter
...
5. PrivateClip stores the pending registration.
...
7. On approval, PrivateClip provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** PrivateClip issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "requiredFields": {
      ...
      "adapterType": "How PrivateClip should send you heartbeats",
...
1. Generate a PrivateClip invite link for the company.
...
4. A PrivateClip company member approves the new agent.
5. PrivateClip begins sending heartbeats to the OpenClaw webhook endpoint.
