...
PrivateClip needs to support two very different but equally valid ways of working:
...
Today, PrivateClip already has the beginnings of this model:
...
- making PrivateClip feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing PrivateClip users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted PrivateClip deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning PrivateClip into a full code review product
...
This object must be recorded explicitly so that PrivateClip can:
...
PrivateClip should treat PRs as a type of work product linked back to:
...
PrivateClip already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted PrivateClip must be a first-class deployment mode

PrivateClip cannot assume that it is running on the same machine as the code.

In cloud deployments, PrivateClip may:
...
- `Runtime service`: a process or service PrivateClip owns or tracks for a workspace
...
It also lets cloud-hosted PrivateClip deployments point at codebases and remotes without pretending the PrivateClip host has direct filesystem access.
...
Without an explicit `execution workspace` record, PrivateClip has nowhere stable to attach:
...
PrivateClip needs a single place to show:
...
- PrivateClip-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without PrivateClip owning the execution host
...
## 1. PrivateClip host and execution host must be decoupled
...
without the PrivateClip host itself running local git or local preview processes.
...
PrivateClip should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for PrivateClip.
...
PrivateClip should keep workspace automation centered on a small set of hooks:
...
PrivateClip should provide the equivalent visibility in the project `Code` surface:
...
PrivateClip should treat execution workspaces as the natural home for:
...
PrivateClip should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
PrivateClip should continue treating cleanup policy as part of the core workspace model:
...
PrivateClip should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the PrivateClip host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

PrivateClip should borrow Worktrunk's good execution patterns while keeping the broader PrivateClip model:
...
This is how PrivateClip supports:
...
That keeps PrivateClip focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted PrivateClip deployments remain first-class

That is a better fit for PrivateClip than either extreme:
