...
`PAP-447` asks how PrivateClip should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same PrivateClip instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into PrivateClip:

- some operators use PrivateClip to manage PrivateClip and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as PrivateClip
- Claude and Codex expose different built-in affordances, so PrivateClip should not overfit to one tool

## Core Product Decision

PrivateClip should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by PrivateClip
...
- PrivateClip should not assume that every adapter can see or use a host filesystem path directly
...
If PrivateClip makes worktrees agent-first, it will blur:
...
- PrivateClip core resolves and records execution workspace state
...
That gives PrivateClip room to support:
...
- PrivateClip handles workspace runtime services under the hood using project/workspace policy
...
Once PrivateClip is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So PrivateClip should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that PrivateClip cannot infer
...
- when issue moves to done, should PrivateClip auto-commit?
- should it auto-push?
...
PrivateClip already has the right foundation for a project-first model.
...
For server-spawned local adapters, PrivateClip already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that PrivateClip should mirror directly

Implication:

- for `codex_local`, PrivateClip should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but PrivateClip should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, PrivateClip should separate:
...
PrivateClip should be able to express intentions such as:
...
These are not specific to PrivateClip. They are a common property of working in a dev workspace, whether local or remote.
...
Do not model this as a PrivateClip-specific `devServer` flag.
...
PrivateClip should distinguish between:
...
PrivateClip should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, PrivateClip should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to PrivateClip
...
For runtime services specifically, PrivateClip should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as PrivateClip-only dev-server flags
...
- same embedded DB / PrivateClip instance can be reused through env/config injection when appropriate
...
- PrivateClip can decide whether to reuse or start a fresh service deterministically
...
3. Test against PrivateClip-on-PrivateClip workflows first.
...
7. Workspace runtime services are modeled generically, not as PrivateClip-specific dev-server toggles.
