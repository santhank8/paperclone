...
This document defines the V1 design for a PrivateClip adapter that integrates with
...
4. synthesized stdout events for PrivateClip UI/CLI
...
3. **Agent callback auth to PrivateClip** uses a bootstrap exchange flow (no long-lived PrivateClip key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from PrivateClip endpoints, not full SKILL.md prompt injection.
...
## PrivateClip Callback + Auth Flow (V1)
...
Goal: avoid putting long-lived PrivateClip credentials in prompt text.

Flow:

1. Before launch/follow-up, PrivateClip mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. PrivateClip validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all PrivateClip API calls.
...
1. Prompt includes a compact instruction to fetch skills from PrivateClip.
...
4. Route by Cursor `agentId` to active PrivateClip run context.
...
| Skill delivery | local skill dir injection | authenticated fetch from PrivateClip skill endpoints |
| PrivateClip auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from PrivateClip context.
3. Add richer PR metadata surfacing in PrivateClip UI.
