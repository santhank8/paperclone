...
This document defines the next-stage plan for PrivateClip company import/export.

The core shift is:

- move from a PrivateClip-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside PrivateClip.
...
PrivateClip already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just PrivateClip
...
7. an always-emitted `.paperclip.yaml` sidecar for high-fidelity PrivateClip-specific details
...
11. adapter-aware skill sync surfaces so PrivateClip can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point PrivateClip at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a PrivateClip sidecar.
...
PrivateClip must not redefine `SKILL.md`.
...
- PrivateClip-specific fidelity lives in `.paperclip.yaml`
- PrivateClip may resolve and install `SKILL.md` packages, but it must not require a PrivateClip-only skill format
...
### 5.4 Base Package Vs PrivateClip Extension
...
- base package:
  ...
  - no PrivateClip-only runtime fields by default
- PrivateClip extension:
  - `.paperclip.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by PrivateClip tools as a sidecar while the base package stays readable
...
PrivateClip import/export should support these entity kinds:
...
- PrivateClip can treat “this set of agents came from team package X” as the imported-team model
...
PrivateClip should support source references in package metadata with:
...
PrivateClip should define a new adapter surface area around skills:
...
Planned PrivateClip behavior:

- if an adapter supports read, PrivateClip should show current skills in the UI
- if an adapter supports write, PrivateClip should let the user enable/disable imported skills
- if an adapter supports sync, PrivateClip should compute desired vs actual state and offer reconcile actions
...
## 10. Storage Model Inside PrivateClip
...
PrivateClip should add managed package/provenance records so imports are not anonymous one-off copies.
...
1. Should imported skill packages be stored as managed package files in PrivateClip storage, or only referenced at import time?
...
3. Should PrivateClip support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps PrivateClip aligned with:
