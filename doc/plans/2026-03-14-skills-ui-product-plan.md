...
This document defines the product and UI plan for skill management in PrivateClip.
...
- PrivateClip company import/export can include skills as package content
...
3. The UI must always tell the truth about what PrivateClip knows:
   - desired state in PrivateClip
...
PrivateClip should treat skills at two scopes:
...
- created locally inside PrivateClip later
...
7. “Understand what PrivateClip knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a PrivateClip-specific package layout
...
This matters because Codex and similar adapters may already have local skills that PrivateClip did not install.

These should be clearly marked:

- external
- not managed by PrivateClip
...
- `desired_only`: PrivateClip wants it, adapter does not show it yet
- `external`: adapter has it but PrivateClip does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, PrivateClip only tracks desired state
...
- PrivateClip can still track desired skills
...
- PrivateClip can see adapter skills
...
- `.paperclip.yaml` may add PrivateClip-specific fidelity, but should not replace the base shortname association model
...
- `.paperclip.yaml` may add PrivateClip-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
PrivateClip should have a company-scoped skill model or managed package model representing:
...
PrivateClip should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too PrivateClip-specific.
...
That gives PrivateClip one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
