...
This document defines the rollout plan for adapter-wide skill support in PrivateClip.
...
- PrivateClip stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

PrivateClip currently has these adapters:
...
1. Desired skills live in PrivateClip for every adapter.
...
These adapters have a stable local skills directory that PrivateClip can read and manage.
...
These adapters do not have a meaningful PrivateClip-owned persistent install state.
...
- show desired PrivateClip skills
...
- desired skills stored in PrivateClip
...
- runtime already injects PrivateClip skills into `~/.cursor/skills`
...
- runtime already injects PrivateClip skills into `~/.gemini/skills`
...
- runtime already injects PrivateClip skills into `~/.pi/agent/skills`
...
- PrivateClip can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects PrivateClip skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- PrivateClip may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed PrivateClip skills that are clearly marked as PrivateClip-managed
...
- PrivateClip stores desired skills only
...
- avoid destructive wording unless PrivateClip can prove a skill is PrivateClip-managed
...
That gets PrivateClip from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
