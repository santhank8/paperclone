...
PrivateClip already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model PrivateClip should implement next.

## Product Goals

PrivateClip should let operators:
...
For the next implementation pass, PrivateClip should enforce these defaults:
...
PrivateClip should separate subscription-included usage from billed spend:
...
PrivateClip should have two threshold classes:
...
PrivateClip must know that a project is budget-paused and enforce it.
...
PrivateClip must also block execution before new work starts.
