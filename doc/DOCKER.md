...
PrivateClip server + PostgreSQL 17. The database is health-checked before the server starts.
...
PostgreSQL data persists in a named Docker volume (`pgdata`). PrivateClip data persists in `paperclip-data`.
...
For authenticated deployments, set one canonical public URL and let PrivateClip derive auth/callback defaults:
...
- Adapter environment checks in PrivateClip will surface missing auth/CLI prerequisites.

## Podman Quadlet (systemd)

The `docker/quadlet/` directory contains unit files to run PrivateClip + PostgreSQL as systemd services via Podman Quadlet.

| File | Purpose |
|------|---------|
| `docker/quadlet/paperclip.pod` | Pod definition — groups containers into a shared network namespace |
| `docker/quadlet/paperclip.container` | PrivateClip server — joins the pod, connects to Postgres at `127.0.0.1` |
...
- Containers in a pod share `localhost`, so PrivateClip reaches Postgres at `127.0.0.1:5432`.
- PostgreSQL data persists in the `paperclip-pgdata` named volume.
- PrivateClip data persists at `~/.local/share/paperclip`.
...
- PrivateClip data persists via Docker volumes/bind mounts (compose) or at `~/.local/share/paperclip` (quadlet).
