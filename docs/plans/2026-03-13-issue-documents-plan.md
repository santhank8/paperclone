...
Add first-class **documents** to PrivateClip as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by PrivateClip workflow/docs.
...
Update the PrivateClip agent workflow so planning no longer edits the issue description.
