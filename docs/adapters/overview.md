...
summary: What adapters are and how they connect agents to PrivateClip
---

Adapters are the bridge between PrivateClip's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, PrivateClip:
...
You can build and distribute adapters as standalone packages — no changes to PrivateClip's source code required. External adapters are loaded at startup via the plugin system.
...
External adapters can ship a self-contained UI parser that tells the PrivateClip web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
