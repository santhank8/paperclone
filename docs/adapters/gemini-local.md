...
`gemini_local` 适配器在本地运行 Google 的 Gemini CLI。它支持通过 `--resume` 的会话持久化、技能注入和结构化 `stream-json` 输出解析。

## 前置条件

- 已安装 Gemini CLI（`gemini` 命令可用）
- 设置了 `GEMINI_API_KEY` 或 `GOOGLE_API_KEY`，或已配置本地 Gemini CLI 认证
...
| `yolo` | boolean | 否 | 传入 `--approval-mode yolo` 以无人值守运行 |

## 会话持久化

适配器在心跳之间持久化 Gemini 会话 ID。下次唤醒时，它使用 `--resume` 恢复现有对话，使智能体保持上下文。

会话恢复感知工作目录：如果工作目录自上次运行以来发生了变化，则启动全新会话。

如果恢复因未知会话错误失败，适配器会自动使用全新会话重试。

## 技能注入

适配器将 Paperclip 技能符号链接到 Gemini 全局技能目录（`~/.gemini/skills`）。现有用户技能不会被覆盖。
