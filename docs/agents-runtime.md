...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), PrivateClip assumes the CLI is already installed and authenticated on the host machine.
...
PrivateClip stores session IDs for resumable adapters.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. PrivateClip surfaces this as a warning in environment tests, not a hard error.
