...
How to get OpenClaw running in a Docker container for local development and testing the PrivateClip OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

PrivateClip includes an end-to-end join smoke harness:
...
- probes and prints a PrivateClip host URL that is reachable from inside OpenClaw Docker
...
If your PrivateClip deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host PrivateClip server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed PrivateClip URL (typically `http://host.docker.internal:3100`).
- If PrivateClip rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm paperclipai allowed-hostname host.docker.internal
```

Then restart PrivateClip and rerun the smoke script.
