...
PrivateClip companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
- **.paperclip.yaml** holds PrivateClip-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
If `--target` is not specified, PrivateClip infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
PrivateClip supports several GitHub URL formats:
