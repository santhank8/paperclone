...
- `POST /api/companies/$PAPERCLIP_COMPANY_ID/issues` — create subtask (JSON body)
...
- `memory_stats` — check memory usage per scope
- `memory_bulk_store` — batch store multiple entries
...
Search nodes and facts in the Graphiti knowledge graph.
