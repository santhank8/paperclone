...
class Agent:
    ...
    def check_model_supports_utilities(
        ...
        if self.tools is not None:
            if len(self.tools) > 2:
                ...
                if out is False:
                    logger.warning(
                        f"[Agent: {self.agent_name}] Model '{self.model_name}' does not support parallel function calling; "
                        f"tool calls will be sequential. Use a parallel function calling-enabled model for better performance."
                    )
    ...
    def call_llm(
        ...
        **kwargs,
        ...
        # Pass tools to LLM when present (e.g. OpenAIDirectLLM needs tools at run() to send tool_calls)
        run_kwargs = dict(kwargs)
        if getattr(self, "tools_list_dictionary", None):
            run_kwargs["tools_list_dictionary"] = self.tools_list_dictionary
            ...
            if self.stream and hasattr(self.llm, "stream"):
                ...
                if img is not None:
                    streaming_response = self.llm.run(
                        task=task, img=img, *args, **run_kwargs
                    )
                else:
                    streaming_response = self.llm.run(
                        task=task, *args, **run_kwargs
                    )
                ...
                out = self.llm.run(**args, **run_kwargs)
    ...
    def execute_tools(self, response: any, loop_count: int):
        ...
        try:
            # When streaming is used, call_llm returns only the aggregated text (no tool_calls).
            # execute_function_calls_from_api_response expects an API-shaped dict with
            # choices[0].message.tool_calls; given a string it returns [] so Tool Execution Results stay empty.
        ...
        if output in (None, [], ()):
            logger.debug(
                "Tool execution output empty (expected when streaming: response has no tool_calls). "
                "Use stream=false in the API request if you need tool execution results."
            )
        self.short_memory.add(
            role="Tool Executor",
